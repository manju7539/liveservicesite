...
class SuperAdminController extends CI_Controller
{
    ...
    public function hotelLists()
    {
        ...
        // $this->SuperAdmin->remove_mode();
            ...
            elseif(!empty($searchon) && empty($date) )
            {
                
                $data['hotel_data'] = $this->SuperAdmin->ser_check_list_hotel($searchon);
             
                // print_r($searchon);die;
                $this->load->view('include/header');
                $this->load->view('superadmin/hotelLists', $data);
                $this->load->view('include/footer');
            }
            elseif(!empty($date) && !empty($searchon))
            {
                
                $data['hotel_data'] = $this->SuperAdmin->searchcCrud($searchon,$date);
    ...
    public function hotel_info()
	{
        ...
        // $data['hotel_data'] = $this->SuperAdmin->getAllDatat($tbl = 'register', $where);
            ...
            // print_r($hotel_information['data'] );die;
			...
			$hotel_information['hotels_pics'] = $this->SuperAdmin->get_hotel_photos($admin_id);
			            //   print_r($hotel_information['hotels_pics']);die;
    ...
    //     $email_data = $this->SuperAdmin->get_email_data('register',$wh);
    ...
    public function update_hotel()
    {
        ...
        if($result)
        {
            // $this->session->set_flashdata('msg','Data Added Succesfully....!');
            $data['hotel_data']= $this->SuperAdmin->searchCrud($where);
            // echo "<pre>";
            // print_r($data);
            // exit;

            // redirect(base_url('hotelLists'));	
    ...
    public function add_hotels()
    {
        ...
        // print_r($this->input->post('u_id'));exit;
        $filename = $_FILES['hotel_logo']['name'];
        $filetype = $_FILES['hotel_logo']['type'];
        if ($filetype == 'image/jpeg' or $filetype == 'image/png' or $filetype == 'image/gif') {
            $path = FCPATH . 'assets/hotel_logo/' . $filename;
            // echo $_FILES['hotel_logo']['tmp_name'];exit;
            // move_uploaded_file($_FILES['hotel_logo']['tmp_name'], $path);
            move_uploaded_file($_FILES['hotel_logo']['tmp_name'], $path);
            // print_r('hello');exit;
    ...
    {  
        ...
        if($result)
        {
            ...
            // $this->session->set_flashdata('credit_msg', 'Data Updated Suceesfully..!');
            // redirect(base_url().'mng_credits'); 
            $wh = '(city_id = "'.$city.'")';
            $data['recharg'] = $this->SuperAdmin->getSingleData('city',$wh);
            $data['leads_recharge'] = $this->SuperAdmin->get_credit_amount();
            $wh = '(u_id = "'.$hotelname.'")';
            $data['catgory'] = $this->SuperAdmin->getSingleData($tbl='register',$wh);
    ...
    public function plan_purchase_hotels()
    {
        ...
        // print_r($buy_plan['plan_purchase_list']);
        // exit;
        $this->load->view('include/header');
    ...
    public function guest_panel()
    {
        ...
        if(!empty($date) && !empty($searchon) )
        {
            ...
            $data['list'] = $this->SuperAdmin->guest_panel_date_filter($date,$searchon);
        ...
        else if (!empty($searchon) ) {
            $data['list'] = $this->SuperAdmin->ser_guest_hotelwise1($searchon);
            $this->load->view('include/header');
            $this->load->view('superadmin/guest_panel', $data);
            $this->load->view('include/footer');
            ...
            $data['list'] = $this->SuperAdmin->guest_date_filter($date);
            $this->load->view('include/header');
            $this->load->view('superadmin/guest_panel', $data);
            $this->load->view('include/footer');
    ...
    {
        $this->load->view('include/header');
        $this->load->view('superadmin/app_privacy');
        $this->load->view('include/footer');
    ...
    public function update_departments()
    {
        ...
        // print_r($data['departement']['icon']); exit;
    ...
    public function add_city()
    {
        ...
        if ($city_data) {
            // $this->session->set_flashdata('msg','City already exits..');
            ...
            if ($add) {
                $data["city"] = $this->SuperAdmin->getPlan($tbl = 'city');
                // print_r($data["city"]);
                // exit;
    ...
    $id          = $this->input->post('notification_id');
			...
			// echo $u_id;die;
                ...
                $admin_id = $admin_details['hotel_id'];
                    ...
                    // echo "<pre>";
                    //  print_r($check_out_view['booking_details']);die;
        ...
        public function customer_view()
        {
            ...
            // $customer_view['booking_id'] = $booking_id;
        ...
        public function resend_notification()
                {   
                   ...
                   // print_r($notificationId);
                    ...
                    // print_r($notification_data1 );die;
                    ...
                    $add = $this->MainModel->insert_data('notifications',$arr);
                    ...
                    if($add)
                    {
                        ...
                        $this->session->set_flashdata('msg','Resent Notification  Successfully...');
                        //redirect(base_url('panel_noti'));
                    }
                    else
                    {
                   ...
                   // $data['list'] = $this->SuperAdmin->get_notification_list();
                   // redirect(base_url('panel_noti'));
                   // echo json_en($getnotificationlist);exit;
                ...
                //    echo "$admin_id" ;die;
                   ...
                   $price          =$this->input->post('price');
                    ...
                    if($result)
                    {
                        // $this->session->set_flashdata('validity_msg', 'Data Update Successfully..!');
                        // redirect(base_url().'SuperAdminController/hotel_info/'.$u_id);
                    ...
                    else
                    {
                        ...
                        // redirect(base_url().'SuperAdminController/hotel_info');
                    }
                }
                ...
                public function get_view_name_data_notification()
                {
                    ...
                    $data['row'] = $this->SuperAdmin->get_notification_list();
