        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card card-topline-aqua">
                    ...
                    <!-- start :: First_menu div -->
                    <div class="bydefalt_div">
                        <div class="table-scrollable">
                            <table id="first_menu_div" class="display full-width">
                                <thead>
                                    <tr>
                                        <th>Sr.no.</th>
                                        <!-- <th>Category </th> -->
                    ...
                    <!-- End :: First_menu div -->
                    <!-- Chiragi start :: without First_menu div -->
                    ...
                    <!-- Chiragi End :: without First_menu div -->
                    </div>
                </div>
            </div>
        </div>
...
<!-- start :: add btn modal for first menu -->
<div class="modal fade bd-add-modal add_facility_modal" tabindex="-1" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <input type="hidden" name="first_menu_id" id="first_menu_id" value="">  
...
<!-- end :: add btn modalfor first menu -->
<!-- start :: add btn modal for second to all menu -->
<div class="modal fade bd-add-modal secont_to_all_menu" tabindex="-1" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="all_menu_form"  method="post" enctype="multipart/form-data">  
                <input type="hidden" name="selected_menu_service_id" id="selected_menu_service_id" value="">  
                <div class="modal-header">
                    <h5 class="modal-title" style="font-weight: bold;font-size: 15px !important;">Add Items</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal">
                    </button>
...
<!-- end :: add btn modal for second to all menu -->
...
<script>
    ...
    var first_menuListDatatable;
    var menuListDatatable;
    $(document).ready(function() {
        var selected_id = $('#categoryDropdown').val();
        $('#first_menu_id').val(selected_id)
        var tb_id = 'first_menu_div';
        first_menuListDatatable = $('#first_menu_div').DataTable({
            "order": [],
            'processing': true,
            'serverSide': true,
            "bDestroy": true,
    ...
    $('#categoryDropdown').change(function () {
        $('#addRow1').removeClass('add_facility');
        $('#addRow1').addClass('remove_furst_menu');
        var id = $(this).val();
        if(id != '')
        {
        ...
        var base_url = '<?php echo base_url();?>';
        var tb_id = 'selected_menus_data';
        menuListDatatable = $('#selected_menus_data').DataTable({
            "order": [],
            'processing': true,
            'serverSide': true,
            "bDestroy": true,
    ...
    $(document).on("click",".remove_furst_menu",function(){
        var selected_menuservice = $(this).attr("data-selected-cat");
        $('#selected_menu_service_id').val(selected_menuservice);
        $(".secont_to_all_menu").modal('show');
    });

    $(document).on('submit', '#all_menu_form', function(e){
        e.preventDefault();
        $(".loader_block").show();
        var form_data = new FormData(this);
        $.ajax({
            ...
            success     : function(res) {
                ...
                // $(".append_data_ajax").html('');
                // $(".append_data").html('');
                $('.secont_to_all_menu').modal('hide');  
                $(".loader_block").hide();
                $('#selected_menus_data').DataTable().ajax.reload();
                // $(".append_data_ajax").append(res)
                setTimeout(function(){  
                    $("#all_menu_form")[0].reset();         
                    $(".successmessage").show();
                }, 20);
                setTimeout(function(){  
                    $(".successmessage").hide();
    ...
    //chiragi End :: edit model open 

    // chiragi start :: second menu after edit listing
    $(document).on('submit','#manumanagement_edit_form',function(e){
        e.preventDefault(); 
        // console.log(this);
        //         return false;
        ...
        $.ajax({
            ...
            success: function (response) {
                $('#menuservice_edit_modal').modal('hide');  
                $(".loader_block").hide();
                if(table_id == 'selected_menus_data')
                {
                    $('#selected_menus_data').DataTable().ajax.reload();
                }
                if(table_id == 'first_menu_div')
                {
                    $('#first_menu_div').DataTable().ajax.reload();
                }
                // $(".append_data_ajax").append(response);
    ...
    // chiragi End :: second menu after edit listing
