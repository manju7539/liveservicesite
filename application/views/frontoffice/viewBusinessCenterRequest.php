<style>
    .reservation_block .container-fluid{
        padding:0px
    }
    a.disabled_data {
  pointer-events: none;
...
<script>
    ...
    //     let intervalId = setInterval(auto_load_business_req, 20000);
    //     function auto_load_business_req() {
    //         var base_url = '<?php echo base_url();?>';

    //         var tb_destroy = function(){
    //             $.get( '<?= base_url('businessCenterRequest');?>', function( data ) {
    //                 var resu = $(data).find('.new_record').html();
    //                 $('.new_record').html(resu);
    //                 setTimeout(function(){
    //                     $('#example1').find('.append_data3').html('');
    //                     $('#example1').DataTable().destroy();
    //                 }, 200);
    //             }); 
    //         };
    //         $.ajax({
    //             url: base_url + "FrontofficeController/get_business_data",
    //             method: "GET",
    //             beforeSend: tb_destroy,
    //             success: function(ans) {
    //                 if(ans != '')
    //                 {
    //                     setTimeout(function(){
    //                         $('#example1').find('.append_data3').append(ans);
    ...
    table_visiters = $('#example1').DataTable({
        "order": [],
        'processing': true,
        'serverSide': true,
        "bDestroy": true,
...
<script>
     $(document).ready(function (id) {
            $(document).on('click','#edit_data',function(){
                ...
                // alert(id);
                $.ajax({
                                          url: '<?= base_url('FrontofficeController/getStaffData') ?>',
                                            type: "post",
                                          data: {id:id},
                                          dataType:"json",
                                          success: function (data) {
                                             
                                             console.log(data.additional_note);
                                             $('#b_c_reserve_id').val(data.b_c_reserve_id);
                                             $('.description_view').html(data.additional_note);                                            //  $('#mobile_no').val(data.mobile_no);
     ...
     $(document).on('submit', '#frmupdateblock', function(e){
        ...
        var form_data = new FormData(this);
        $.ajax({
            url         : '<?php echo base_url('FrontofficeController/change_user_status') ?>',
            type      : 'POST',
            data        : form_data,
            processData : false,
            contentType : false,
...
{
 $.ajax({
  url:"<?php echo base_url(); ?>FrontofficeController/businessCenterRequest",
  method:"POST",
  data:{query:query},
  success:function(data){
