                                 ...
                                 <?php 
                                    if(!empty($completed))
                                    {
                                        ...
                                        foreach($completed as $com)
                                        {
                                            ...
                                            $user_com = $this->HouseKeepingModel->getData('register',$wh_admin);
...
<script>
   $(document).ready(function (id) {
      $(document).on('click','#reassign_data',function(){
         ...
         $.ajax({
                  url: '<?= base_url('HouseKeepingController/getreassigndata') ?>',
                     type: "post",
                  data: {id:id},
                  dataType:"json",
                  success: function (data) {
                     ...
                     $('#status option[value="' + data.create_staff_id + '"]').remove();
                  }
      }); 
   })
   });
   $(document).on('submit', '#frmupdateblock1', function(e){
         ...
         $.ajax({
             url         : '<?php echo base_url('HouseKeepingController/reassign_status') ?>',
             type      : 'POST',
             data        : form_data,
             processData : false,
             contentType : false,
             ...
             success     : function(res) {
               $.get( '<?= base_url('staff_handover');?>', function( data ) {
