...
public function user_registration()
{
    // Check if all required parameters except id_proff_img are present
    if (!empty($this->input->post('name')) &&
        !empty($this->input->post('mobile_no')) &&
        !empty($this->input->post('email')) &&
        !empty($this->input->post('password')) &&
        !empty($this->input->post('city'))) 
    {
        ...
        if (empty($check_mobile_no)) {
            if (empty($check_email_id)) {
                $img_url = NULL; // Default image URL to NULL

                // Check if an image is uploaded
                if (!empty($_FILES['id_proff_img']['tmp_name'])) {
                    // Define upload directory
                    $upload_dir = 'assets/upload/id_proff_pic_for_add_booking/';
                    if (!is_dir($upload_dir)) {
                        mkdir($upload_dir, 0777, true); // Create directory if not exists
                    ...
                    // Move uploaded file to the directory
                    if (move_uploaded_file($_FILES['id_proff_img']['tmp_name'], $file_path)) {
                        $img_url = base_url($file_path); // Get the full URL of the uploaded image
                    }
                }
                ...
                $data = [
                    ...
                    'city' => $city,
                    'register_date' => date('Y-m-d'),
                    'Id_proff_photo' => $img_url, // This can be NULL if no image was uploaded
                    'login_type' => 1, 
                ];
        ...
        // Identify missing parameters
        $missing_params = [];
        foreach (['name', 'mobile_no', 'email', 'password', 'city'] as $param) {
      ...
      public function user_login()
{
    ...
    exit();
                    ...
                    if($enquiry_data)
                    {
                        ...
                        foreach ($enquiry_data as $eq) 
                        {
                            ...
                            if($sub_enquiry_data)
                            {
                                ...
                                foreach ($sub_enquiry_data as $s_eq) 
                                {
                                    ...
                                    $sub_enq_data[] = array(
                                                                ...
                                                                'hotel_id' => $s_eq['hotel_id'],  // Added hotel ID
                            ...
                            $send_data[] = array(
                                                    ...
                                                    'hotel_id' => $eq['hotel_id'], // Added hotel ID
                                                    'city' => $city,
       ...
       /* public function verify_otp_for_guest_login()
            {
                // Load input values safely
                $guest_login_id = $this->input->post('guest_login_id', true);
                $otp = $this->input->post('otp', true);
            
                // Validate required parameters
                if (empty($guest_login_id) || empty($otp)) {
                    $response = [
                        'error_code' => "406",
                        'message' => "Required Parameter Missing..!"
                    ];
                ...
                // Fetch guest user securely
                $wh = array('guest_user_id' => $guest_login_id, 'otp' => $otp);
                $guest_user = $this->ApiModel->getData('guest_user', $wh);
            
                // If no guest user found, return error
                if (!$guest_user) {
                    $response = [
                        'error_code' => "404",
                        'message' => "Login ID and OTP do not match"
                    ];
                    echo json_encode($response);
                    exit();
                }
                ...
                $update_data = ['is_otp_verified' => 1];
                $update_status = $this->ApiModel->edit_data('guest_user', ['guest_user_id' => $guest_login_id], $update_data);
            
                // Check-in status update conditions
                $wh1 = array(
                ...
                $update_checkin = [
                    'check_in_status' => 1,
                    'id_proff_img' => isset($user['Id_proff_photo']) ? $user['Id_proff_photo'] : null
                ];
            
                $checkin_status = $this->ApiModel->edit_checkin_status('user_hotel_booking', $wh1, $update_checkin);
            
                // Determine verification status
                $verify_id_status = !empty($checkin_status['id_proff_img']) ? 1 : 0;
            
                if ($update_status && $checkin_status) {
                    $response = [
                        'error_code' => "200",
...
public function verify_otp_for_guest_login()
{
    ...
    // Fetch guest user securely
    $wh = ['guest_user_id' => $guest_login_id, 'otp' => $otp];
    $guest_user = $this->ApiModel->getData('guest_user', $wh);
    ...
    // Updated verification status logic
    $verify_id_status = !empty($user['Id_proff_photo']) ? 1 : 0;
       ...
       public function get_help_support_list()
{
    if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id'))) {
        $u_id = $this->input->post('u_id');
        $hotel_id = $this->input->post('hotel_id');
        $booking_id = $this->input->post('booking_id') ?? null; // Optional booking_id
        ...
        // If booking_id is provided, validate guest session
        if (!empty($booking_id)) {
            $guest_user = $this->ApiModel->get_user_guest_data($u_id, $hotel_id, $booking_id);
            if (!$guest_user) {
        ...
        if (!$help_support_list) {
            echo json_encode(['error_code' => "404", 'message' => "Data Not Found"]);
            exit();
        }
        ...
        public function get_hotel_near_by_places_list()
{
    if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id'))) {
        ...
        // If booking_id is provided, check guest login session, otherwise skip
        if (!empty($booking_id)) {
            $guest_user = $this->ApiModel->get_user_guest_data($u_id, $hotel_id, $booking_id);
            if (!$guest_user) {
                echo json_encode(['error_code' => "404", 'message' => "Guest Login session expired"]);
        ...
        foreach ($near_by_places_list as $h_plac) {
            ...
            $send_data[] = [
                ...
                'places' => $h_plac['places'],
                'places_name' => $h_plac['places_name'],
                'image' => $near_by_places_img['images'] ?? "", // Handle missing images
            ];
        }
...
{
    if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id')) && !empty($this->input->post('hotel_near_by_place_id'))) {
        $u_id = $this->input->post('u_id');
        $hotel_id = $this->input->post('hotel_id');
        $hotel_near_by_place_id = $this->input->post('hotel_near_by_place_id');
        $booking_id = $this->input->post('booking_id') ?? null; // Optional
        ...
        // Validate guest session only if booking_id is provided
        if (!empty($booking_id)) {
            $guest_user = $this->ApiModel->get_user_guest_data($u_id, $hotel_id, $booking_id);
            if (!$guest_user) {
                echo json_encode(['error_code' => "404", 'message' => "Guest Login session expired"]);
        ...
        // Fetch images
        $images = [];
        $where_condition = '(hotel_near_by_place_id = "' . $near_by_places_details['hotel_near_by_place_id'] . '")';
        $near_by_places_imgs = $this->ApiModel->getAllData('hotel_near_by_place_images', $where_condition);
        ...
        $send_data = [
            ...
            'latitude' => $near_by_places_details['latitute'],
        ...
        echo json_encode([
            'error_code' => "200",
            'message' => "Data Found",
            'data' => [$send_data] // Ensures consistent response format
        ]);
        exit();
    }
        ...
        public function get_hotel_near_by_help_list()
{
    if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id'))) 
    {
        $u_id = $this->input->post('u_id');
        $hotel_id = $this->input->post('hotel_id');
        $booking_id = $this->input->post('booking_id') ?? null; // Booking ID is optional

        $user = $this->ApiModel->get_user_data($u_id);

        if ($user) 
        {
            // If booking_id is provided, validate the guest session, otherwise skip
            if ($booking_id) {
                $guest_user = $this->ApiModel->get_user_guest_data($u_id, $hotel_id, $booking_id);
            ...
            if ($near_by_help_list) 
            {
                ...
                foreach ($near_by_help_list as $n_help) 
                {
                    ...
                    $images = $near_by_help_img ? $near_by_help_img['images'] : "";
...
{
    if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id')) && !empty($this->input->post('hotel_near_by_help_id'))) 
    {
        $u_id = $this->input->post('u_id');
        $hotel_id = $this->input->post('hotel_id');
        $booking_id = $this->input->post('booking_id') ?? null; // Booking ID is optional
        ...
        if ($user) 
        {
            // If booking_id is provided, check guest session, otherwise skip
            if ($booking_id) {
                $guest_user = $this->ApiModel->get_user_guest_data($u_id, $hotel_id, $booking_id);
       ...
       public function get_hotel_guidelines()
{
    if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id'))) {
        ...
        echo json_encode([
            'error_code' => "200",
            'message' => "Data Found",
            'data' => [$send_data] // Keeping response format consistent
        ]);
        exit();
    }
    ...
    if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id')) && !empty($this->input->post('booking_id')) && !empty($this->input->post('review_data'))) {
        ...
        if ($user) {
            ...
            if ($guest_user) {
                ...
                $existing_review = $this->ApiModel->get_existing_review($u_id, $hotel_id, $booking_id);

                if ($existing_review) {
                    // Update the existing review
                    $update_data = [
                        'rating' => $review_data[0]['rating'],
                        'review_for' => $review_data[0]['review_for'],
                        'review' => $review,
                        'rating_date' => date('Y-m-d'),
                        'rating_time' => date('H:i:s'),
                    ];

                    $update_status = $this->ApiModel->update_review($existing_review->id, $update_data);
                    ...
                    $insert_status = $this->ApiModel->insert_data('review', $insert_data);
     ...
     public function sent_request_to_luggage()
        {
            ...
            if (!empty($this->input->post('u_id')) && 
                ...
                !empty($this->input->post('select_date')) && 
                !empty($this->input->post('select_time'))) 
            {
                $u_id = $this->input->post('u_id');
                $hotel_id = $this->input->post('hotel_id');
                $booking_id = $this->input->post('booking_id');
                ...
                // Make luggage_type optional
                $luggage_type = isset($_POST['luggage_type']) ? $this->input->post('luggage_type') : null;
                ...
                if ($user) {
                    ...
                    if ($guest_user) {
                        ...
                        $arr = [
                            ...
                            'select_date' => $select_date,
                            'select_time' => $select_time,
                            'luggage_type' => $luggage_type, // Can be null
                        ...
                        if ($add) {
                            ...
                            $send_data[] = [
                                ...
                                'quantity' => $quantity,
                                'luggage_type' => $luggage_type, // Optional
                                'select_date' => $select_date,
                                'select_time' => date('h:i A', strtotime($select_time)),
                            ];
                        ...
                        $response = [
                            'error_code' => "404",
                            'message' => "Guest Login session expired"
                        ];
                    }
                } else {
                    $response = [
       ...
       public function get_our_hotel_gym_services_details()
{
    if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id'))) {
        ...
        if ($user) {
            // If booking_id is provided, validate the guest session
            if ($booking_id) {
                $guest_user = $this->ApiModel->get_user_guest_data($u_id, $hotel_id, $booking_id);
                if (!$guest_user) {
                    echo json_encode(['error_code' => "404", 'message' => "Guest login session expired"]);
            ...
            if ($hotel_gym_service_data) {
                ...
                $hotel_gym_images = $this->ApiModel->getAllData('front_ofs_services_images', $wh);

                $image_data = [];
                if ($hotel_gym_images) {
                    foreach ($hotel_gym_images as $gym_img) {
                        $image_data[] = [
       ...
       public function get_our_hotel_pool_services_details()
            {
                if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id'))) {
                    $u_id = $this->input->post('u_id');
                    $hotel_id = $this->input->post('hotel_id');
                    $booking_id = $this->input->post('booking_id') ?? null; // Optional parameter
            
                    $user = $this->ApiModel->get_user_data($u_id);
            
                    if ($user) {
                        // If booking_id is provided, check guest user, otherwise skip the check
                        if (empty($booking_id) || $this->ApiModel->get_user_guest_data($u_id, $hotel_id, $booking_id)) {
                            $service_name = 3;
        ...
        public function get_our_hotel_baby_care_details()
            {
                if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id'))) {
                    ...
                    if ($user) {
                        // If booking_id is provided, validate guest session
                        $guest_user = $booking_id ? $this->ApiModel->get_user_guest_data($u_id, $hotel_id, $booking_id) : true;
        ...
        if ($user) {
            ...
            if ($guest_user) {
                ...
                if ($shuttle_service_data) {
                    ...
                    $hotel_shuttle_service_images = $this->ApiModel->getData('front_ofs_services_images', $wh1);

                    $image = $hotel_shuttle_service_images ? $hotel_shuttle_service_images['image'] : "";

                    // Determine day for availability based on flag
                    $day = ($flag == 1) ? date('l', strtotime('tomorrow')) : date('l');
                    $wh2 = "(hotel_id = '$hotel_id' AND front_ofs_service_id = '{$shuttle_service_data['front_ofs_service_id']}' AND day = '$day')";

                    $availability_chart = $this->ApiModel->getAllData('shuttle_service_avaibility', $wh2);

                    $today_available_chart = [];
                    if ($availability_chart) {
                        foreach ($availability_chart as $av_ch) {
                            $time = date('h:i a', strtotime($av_ch['start_time'])) . " - " . date('h:i a', strtotime($av_ch['end_time']));
                            $available_status = ($av_ch['available_status'] == 1) ? "Available" : "Unavailable";
...
{
    // Check if required parameters are provided (booking_id is optional)
    if (!empty($this->input->post('u_id')) && !empty($this->input->post('hotel_id')))
    {
        ...
        if ($user)
        {
            ...
            if ($food_offer_list)
            {
                ...
                // Loop through the offers and prepare the response data
            ...
            {
                // No offers found
                echo json_encode(['error_code' => "404", 'message' => "No offers found"]);
                exit();
            }
        }
        else
    ...
    else
    {
        ...
        echo json_encode(['error_code' => "406", 'message' => "Required parameter(s) missing!"]);
        exit();
    }
}
...
public function get_city_list()
{
    if (!empty($this->input->post('u_id'))) {
        ...
        if ($user) {
            // Fetch only city_id = 18
            $city = $this->ApiModel->get_city_by_id();

            if (!empty($city)) { // Ensure data exists
                $response = [
                    'error_code' => "200",
                    'message' => "Data Found",
                    'data' => $city
    ...
    public function get_my_booking_history()
{
    if (!empty($this->input->post('u_id'))) {
            ...
            if ($user_booking_history) {
                ...
                foreach ($user_booking_history as $h_book) {
                    ...
                    foreach ($get_spa_service_request_data as $g_s_s_r_d) {
                        $front_office_total += $g_s_s_r_d['charges'];
                    }
                    ...
                    $images = $hotel_photos ? $hotel_photos['images'] : "";
                    ...
                    $send_data[] = array(
                        ...
                        'cancel_message' => $cancel_message, // Added cancel message
                    );
                }
      ...
      public function reject_booking_api()
{
    header('Content-Type: application/json');
...
public function cancel_booking_by_user() {
    ...
    // Check if booking exists and get booking status
    $this->db->select('uhb.booking_status');
$this->db->from('user_hotel_booking uhb');
$this->db->join('hotel_enquiry_request her', 'uhb.enquiry_id = her.hotel_enquiry_request_id', 'inner');
$this->db->where('uhb.booking_id', $booking_id);
$this->db->where('uhb.hotel_id', $hotel_id); 
// Remove $this->db->where('uhb.user_id', $user_id);
...
$query = $this->db->get();
    ...
    if ($booking->booking_status != 0) {
        echo json_encode(["status" => "Your cancel request in progress "]);
        return;
    }

    // Proceed with cancellation (Set status to canceled)
    $this->db->where('booking_id', $booking_id);
    $this->db->update('user_hotel_booking', ['booking_status' => 4]); // Assuming 2 = Canceled

    echo json_encode(["status" => "Your cancel request pending.,Please wait..."]);
}
