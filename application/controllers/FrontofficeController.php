...
class FrontofficeController extends CI_Controller
{
    ...
    public function change_check_in_guest()
                {
                    ...
                    $wh = '(check_in = "'.$cat.'" AND booking_status=1 AND check_out >=  "'. $dat.'")';
                    
                    $sub_cat = $this->MainModel->getAllData('user_hotel_booking',$wh);

                    $output = '<option>--Select Guest--</option>'; 
                    ...
                    foreach($sub_cat as $s_c)
                    {
                        ...
                        $output .= '<option value = "'.$s_c['u_id'].'">'.$name.'</option>';
                    }
                    echo $output;
                }
    public function getEditRoomConfigData()
    {
        ...
        $get_hotel_id = $this->MainModel->getData('register', $wh_admin);
        $admin_id = $get_hotel_id['hotel_id'];
        $where = $this->input->post('id'); // $room_type = 1;
        // $guest_mng['floor_list'] = $this->FrontofficeModel->get_floor_list($admin_id);

        // $guest_mng['room_type'] = $this->FrontofficeModel->get_room_type_list1($admin_id);
        // $guest_mng['room_type_data'] = $this->FrontofficeModel->get_room_type_data($admin_id);
        // $guest_mng['list'] = $this->FrontofficeModel->get_room_related_data($admin_id,$room_type);
        // $guest_mng['floor_list1'] = $this->FrontofficeModel->get_floor_list($admin_id);

        $wh = '(room_configure_id ="' . $where . '")';
        $data = $this->MainModel->getSingleData($tbl = 'room_configure', $wh);
        $data['room_number'] = $this->FrontofficeModel->get_room_numbers($admin_id, $where);
        $data['room_facility'] = $this->FrontofficeModel->get_room_facility($admin_id, $where);
        $data['room_imgs'] = $this->FrontofficeModel->get_room_imgs($admin_id, $where);
    ...
    public function delete_business_center()
    {
        ...
        $del = $this->FrontofficeModel->delete_data('business_center', $wh);

        $del = $this->FrontofficeModel->delete_data('business_center_facility', $wh);

        $del = $this->FrontofficeModel->delete_data('business_center_images', $wh);

        if ($del) {
            $admin_id = $this->session->userdata('u_id');

            $guest_mng["list"] = $this->FrontofficeModel->get_business_center_pagination($admin_id);
            $this->load->view('frontoffice/ajaxviewManageBusinessCenter', $guest_mng);
        } else {
            echo "0";
        }
    }
    public function change_new_spa_status()
    {
        ...
        $spa_u_id = $this->input->post('spa_u_id');
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        ...
        if ($request_status == 1) {
            ...
            $up = $this->FrontofficeModel->edit_data('spa_service_request_list', $wh, $arr);
            ...
            if ($up) {
                ...
                $guest_mng["spa_request"] = $this->FrontofficeModel->get_spa_request_list_pagination($admin_id);

                $subject = $title;
                $description = "$title In $hotel_name And Your Spa Service Id Is $spa_service_request_id";
                $arr_noti = array(
                    'hotel_id' => $admin_id,
                    'u_id' => $spa_u_id,
                    'subject' => $subject,
                ...
                $this->load->view('frontoffice/ajaxnewsparequestadd', $guest_mng);
            } else {
                $this->session->set_flashdata('msg', 'Something went wrong');
                redirect(base_url('FrontofficeController/frontOfficeList'));
            }
            ...
            $up = $this->FrontofficeModel->edit_data('spa_service_request_list', $wh1, $arr1);
            ...
            if ($up) {
                ...
                if ($get_u_id['reject_reason'] == 1) {
                    $rejct_reason = "Please Try After Sometime";
                } else if ($get_u_id['reject_reason'] == 2) {
                    $rejct_reason = "Temporarily unavailable";
                } else if ($get_u_id['reject_reason'] == 3) {
                    $rejct_reason = "Slots not available";
                } else if ($get_u_id['reject_reason'] == 4) {
                    $rejct_reason = "Please contact Front office";
                } else {
                    $rejct_reason = "";
                }
                if ($get_dt) {
                    ...
                    $body = 'Your Spa Request ID is "' . $spa_service_request_id . '" And Your Request is Rejected Because OF "' . $rejct_reason . '"';
                    $result = send_push_notification($deviceToken, $title, $body);
                }
                // print_r($result);die;
                ...
                $guest_mng["spa_request"] = $this->FrontofficeModel->get_spa_request_list_pagination($admin_id);
                $subject = $title;
                $description = "$title In $hotel_name And Your Spa Service Id Is $spa_service_request_id And Your Request is Rejected Because OF $rejct_reason ";
                $arr_noti = array(
                    'hotel_id' => $admin_id,
                    'u_id' => $spa_u_id,
                    'subject' => $subject,
    ...
    public function getrequirement_luggage()
    {
        ...
        $guest_mng["luggage_request"] = $this->FrontofficeModel->get_lugggage_request_list1($hotel_id, $id);
        // echo "<pre>";print_r($guest_mng["luggage_request"]);exit;
    ...
    public function change_new_luggage_status()
    {
        ...
        $luggage_u_id = $this->input->post('luggage_u_id');
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        $admin_id = $admin_details['hotel_id'];
        ...
        if ($request_status == 1) {
            ...
            $up = $this->FrontofficeModel->edit_data('luggage_request', $wh, $arr);
            ...
            if ($up) {
                ...
                $subject = $title;
                $description = "$title In $hotel_name And Your Luggage Service Id Is $luggage_request_id";
                $arr_noti = array(
                    'hotel_id' => $admin_id,
                    'u_id' => $luggage_u_id,
                    'subject' => $subject,
                ...
                $this->load->view('frontoffice/ajax_luggage_request', $guest_mng);
            } else {
                $this->session->set_flashdata('msg', 'Something went wrong');
                redirect(base_url('FrontofficeController/frontOfficeList'));
            }
            ...
            $up = $this->FrontofficeModel->edit_data('luggage_request', $wh1, $arr1);
            ...
            if ($up) {
                ...
                if ($get_u_id['reject_reason'] == 1) {
                    ...
                    $rejct_reason = "Space Not Available";
                } else if ($get_u_id['reject_reason'] == 3) {
                    $rejct_reason = "Please Contact Front Office";
                } else if ($get_u_id['reject_reason'] == 4) {
                    $rejct_reason = "Available Post Checkout";
                } else {
                    $rejct_reason = "";
                }
                if ($get_dt) {
                    ...
                    $body = 'Your Luggage Request ID is "' . $luggage_request_id . '" And Your Request is Rejected Because OF "' . $rejct_reason . '"';
                    $result = send_push_notification($deviceToken, $title, $body);
                }

                $guest_mng["luggage_request"] = $this->FrontofficeModel->get_luggage_request_list_pagination($admin_id);
                $subject = $title;
                $description = "$title In $hotel_name And Your Luggage Service Id Is $luggage_request_id And Your Request is Rejected Because OF $rejct_reason ";
                $arr_noti = array(
                    'hotel_id' => $admin_id,
                    'u_id' => $luggage_u_id,
                    'subject' => $subject,
    ...
    public function ajaxspaIconView_v1()
    {
        ...
        $guest_mng['room_type_list'] = $this->FrontofficeModel->get_room_type_list($admin_id);
        $guest_mng["spa_request"] = $this->FrontofficeModel->get_spa_request_list_pagination($admin_id);
        $guest_mng["spa_request1"] = $this->FrontofficeModel->get_spa_accept_list_pagination($admin_id);
        $guest_mng["spa_request6"] = $this->FrontofficeModel->get_spa_complete_list_pagination($admin_id);
        $guest_mng["spa_request2"] = $this->FrontofficeModel->get_spa_reject_list_pagination($admin_id);

        $this->load->view('frontoffice/ajaxFrontSubIconBlockView', $guest_mng);

    }
    public function frontoffice_notification()
    {
        $admin_id = $this->session->userdata('u_id');
        $wh_admin = '(u_id ="' . $admin_id . '")';
        $get_hotel_id = $this->MainModel->getData('register', $wh_admin);
        ...
        $specific_hotel_notis_from_SA = $this->FrontofficeModel->specific_hotel_notis_from_SA($hotel_id);
        $get_front_ofs_notis = $this->FrontofficeModel->get_notifications_for_front_ofs($hotel_id);
    ...
    public function geteditgymData()
    {
        ...
        $guest_mng = $this->FrontofficeModel->getgymsingledata($tbl = 'front_ofs_services', $where);

        $service_name = 1;
        $guest_mng["list"] = $this->FrontofficeModel->get_front_ofs_services_pagination($hotel_id, $service_name);
    ...
    public function getData1()
    {
        ...
        $where = $this->input->post('id');
        $guest_mng["list"] = $this->FrontofficeModel->get_spa_request_list1($hotel_id, $where);

        // print_r($data);die;
        echo json_encode($guest_mng);

    }
    public function geteditspaData()
    {
        $u_id = $this->session->userdata('u_id');
        $wh_admin = '(u_id ="' . $u_id . '")';
        $get_hotel_id = $this->MainModel->getData('register', $wh_admin);
        ...
        // print_r($wh);die;
        $wh = '(front_ofs_service_id ="' . $where . '")';
        $guest_mng["data"] = $this->FrontofficeModel->getspasingledata($tbl = 'front_ofs_services', $where);
    ...
    public function change_cab_service_request()
    {
        ...
        $cab_u_id = $this->input->post('cab_u_id');
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        $admin_id = $admin_details['hotel_id'];
        ...
        // print_r($arr);exit;
        $up = $this->FrontofficeModel->edit_data('cab_service_request_list', $wh, $arr);
        ...
        if ($up) {
            if ($request_status1 == 1) {
                ...
                // print_r($result);die;
                $subject = $title;
                $description = "$title In $hotel_name And Your Cab Service Id Is $cab_service_request_id";
                $arr_noti = array(
                    'hotel_id' => $admin_id,
                    'u_id' => $cab_u_id,
                    'subject' => $subject,
                ...
                if (!empty($get_dt)) {
                    ...
                    $body = 'Your Cab Service Request ID is "' . $cab_service_request_id . '" And Your Request is Rejected Because OF "' . $rejct_reason . '"';
                    $result = send_push_notification($deviceToken, $title, $body);
                }
                $subject = $title;
                $description = "$title In $hotel_name And Your Luggage Service Id Is $luggage_request_id And Your Request is Rejected Because OF $rejct_reason ";
    ...
    public function spa_status()
    {
        ...
        $u_id = $_POST['uid'];
        $where = '(spa_service_request_id="' . $u_id . '")';
        $id = $this->FrontofficeModel->editData($tbl = "spa_service_request_list", $where, $arr);
        $get_u_id1 = $this->MainModel->getData($tbl = 'spa_service_request_list', $where);
        // $wh1 = '(u_id = "'.$get_u_id['u_id'].'")';
        $u_id_spa = $get_u_id1['u_id'];
        $hotel_id_spa = $get_u_id1['hotel_id'];
        // print_r($u_id_spa);exit;
        if ($id) {
            ...
            if (!empty($get_dt)) {
                ...
                $body = 'Your Spa Request ID is "' . $u_id . '"';
                $result = send_push_notification($deviceToken, $title, $body);
            }
            $subject = $title;
            $description = "$title In $hotel_name And Your Completed Spa Service Id Is $u_id";
            $arr_noti = array(
                'hotel_id' => $hotel_id_spa,
                'u_id' => $u_id_spa,
                'subject' => $subject,
                'description' => $description,
                'clear_flag' => 1,
    ...
    public function add_rooms()
    {
        ...
        if ($room_arr) {
            ...
            foreach ($room_arr as $key => $value) {
                ...
                $room_no_data = $this->MainModel->getData('room_nos', $wh);

                if ($room_no_data) {
                    $this->session->set_flashdata('msg', 'Room no Already exits..');
                    redirect(base_url('roomConfig'));
                }
            }
        }
        ...
        if (empty($room_no_data)) {
            ...
            // var_dump( $arr  );die;
            $add = $this->FrontofficeModel->insert_data('room_configure', $arr);
            ...
            if ($add) {
                ...
                for ($i = 0; $i < $count_img; $i++) {
                    if (!empty($_FILES['images']['name'][$i])) {
                        ...
                        if ($this->upload->do_upload('my_uploaded_file')) {
                            ...
                            $this->FrontofficeModel->insert_data('room_imgs', $arr1);
                        }
                    }
                }
                ...
                foreach ($facility_string_arr as $key => $value) {
                    ...
                    $this->FrontofficeModel->insert_data('room_facility', $arr_fl);
                }
                ...
                foreach ($room_arr as $key => $value) {
                    ...
                    $add_room_no = $this->FrontofficeModel->insert_data('room_nos', $arr_rm);
                    ...
                    if ($add_room_no) {
                        ...
                        $this->FrontofficeModel->insert_data('room_status', $arr_rm);
    ...
    public function update_room()
    {
        ...
        $old_room_no = $this->FrontofficeModel->getAllData('room_nos', $wh);

        // $abc= $old_room_no;
        $room_data = $_POST['room_no'];
        $room_arr = explode(",", $room_data);
        if (!empty($_POST['room_no'])) {
            $del = $this->FrontofficeModel->delete_data('room_nos', $wh);
            ...
            if ($room_arr) {
                ...
                foreach ($room_arr as $key => $value) {
                    ...
                    $room_no_data = $this->FrontofficeModel->getData('room_nos', $wh1);
                    ...
                    if ($room_no_data) {
                        foreach ($old_room_no as $o_r_n) {
                            ...
                            $add__old_room_no = $this->FrontofficeModel->insert_data('room_nos', $arr_old_room);
                        }
                        $this->session->set_flashdata('msg', '"' . $value . '"Room no Already exits..');
                        redirect(base_url('FrontofficeController/view_config/' . $room_type));
                    } else {
                        $delete_room = array();
                        foreach ($old_room_no as $o_r_n) {
                            $room_data = $_POST['room_no'];
                        ...
                        if ($delete_room) {
                            // print_r($delete_room);die;
                            foreach ($delete_room as $d_r) {
                                $wh5 = 'hotel_id = "' . $admin_id . '" AND room_no = "' . $d_r . '" AND room_status = 2';
                                $check_occupied = $this->FrontofficeModel->getData('room_status', $wh5);
                                ...
                                if ($check_occupied) {
                                    ...
                                    $delete = $this->FrontofficeModel->delete_data('room_status', $wh6);
                                }
                            }
                        }
                    }
            ...
            if (empty($room_no_data)) {
                ...
                $up = $this->FrontofficeModel->edit_data('room_configure', $wh, $arr);
                ...
                if ($up) {
                    ...
                    if (isset($_POST['room_no'])) {
                        ...
                        if ($del) {
                            ...
                            foreach ($room_arr as $key => $value) {
                                ...
                                $room_status = $this->FrontofficeModel->getData('room_status', $wh2);
                                // print_r($room_status);
                                if ($room_status == null) {
                                    // echo "hi";die;
                                    if ($add_room_no) {
                                    ...
                                    if ($room_status_id != null) {
                                        ...
                                        // print_r($arr_rm);exit;
                                        $this->FrontofficeModel->edit_data('room_status', $wh2, $arr_rm);
                                ...
                                //     );
                                //     // print_r($arr_rm);exit;
                                //     $this->FrontofficeModel->edit_data('room_status',$wh1 ,$arr_rm);
                                // }
                    ...
                    if (isset($_POST['facility'])) {
                        $del1 = $this->FrontofficeModel->delete_data('room_facility', $wh);
                        ...
                        if ($del1) {
                            ...
                            foreach ($room_fc_arr as $key => $value1) {
                                ...
                                $this->FrontofficeModel->insert_data('room_facility', $arr_fc);
                            }
                        }
                    }
    ...
    public function add_room()
    {
        ...
        $floor_data = $this->FrontofficeModel->getData('room_configure', $whf);

        if ($floor_data) {
            $this->session->set_flashdata('msg', 'On This Floor Room Is Already Created Plz View..');
            redirect(base_url('FrontofficeController/roomConfig'));
        } else {
            if ($room_arr) {
                $room_no_data = array();
                ...
                foreach ($room_arr as $key => $value) {
                    $wh = '(hotel_id = "' . $admin_id . '" AND room_no = "' . $value . '")';
                    $room_no_data = $this->FrontofficeModel->getData('room_nos', $wh);
    ...
    public function get_floor_data_in_dashboard_page1()
    {
    ...
    public function frontaArrival()
    {
        ...
        if (!empty($date)) {
            ...
            $digitalcheck_in['list1'] = $this->FrontofficeModel->get_user_upcoming_booking_list_from_app_pagination($admin_id);
            $digitalcheck_in['room_type_list1'] = $this->FrontofficeModel->get_room_type_list($admin_id);
            $digitalcheck_in['today_hotel_book_list_by_app1'] = $this->FrontofficeModel->get_upcoming_list_from_app($admin_id);
            $digitalcheck_in['walking_guest_for_sign1'] = $this->FrontofficeModel->get_upcoming_list_from_app($admin_id);
    ...
    public function today_arrival_autoload()
    {
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        ...
        if (!empty($date)) {
            $digitalcheck_in['today_hotel_book_list_by_app'] = $this->FrontofficeModel->ser_check_list($date);
            $this->load->view('frontoffice/ajax_auto_today_FrontaArrival', $digitalcheck_in);
        } else {
            $digitalcheck_in['today_hotel_book_list_by_app'] = $this->FrontofficeModel->get_user_pending_booking_list_from_app($admin_id);
            $this->load->view('frontoffice/ajax_auto_today_FrontaArrival', $digitalcheck_in);
        }
...
// print_r($date);exit;
    ...
    public function businessCenterRequest()
    {
        ...
        $searchon = $this->input->post('business_center_type');
        if (!empty($date) && !empty($searchon)) {
            $u_id = $this->session->userdata('u_id');
            $where = '(u_id = "' . $u_id . '")';
            $admin_details = $this->FrontofficeModel->getData($tbl = 'register', $where);
            ...
            $data['list'] = $this->FrontofficeModel->searchcCrud($searchon, $hotel_id, $date);
            ...
            $data['business_center'] = $this->FrontofficeModel->get_active_business_center_app($admin_id);

            $data["list1"] = $this->FrontofficeModel->get_business_center_list_pagination($admin_id);
            $data['business_center1'] = $this->FrontofficeModel->get_active_business_center($admin_id);

            $data["list2"] = $this->FrontofficeModel->get_business_center_list_reject_pagination($admin_id);
            $data['business_center'] = $this->FrontofficeModel->get_active_business_center($admin_id);
			$this->load->view('include/header');
            $this->load->view('frontoffice/business_reser', $data);
            $this->load->view('include/footer');
        } else {
            ...
            $business_center_reservation["list1"] = $this->FrontofficeModel->get_business_center_list_pagination($admin_id);
            $business_center_reservation['business_center1'] = $this->FrontofficeModel->get_active_business_center($admin_id);

            $business_center_reservation["list2"] = $this->FrontofficeModel->get_business_center_list_reject_pagination($admin_id);
            $business_center_reservation['business_center'] = $this->FrontofficeModel->get_active_business_center($admin_id);
            // print_r($business_center_reservation['business_center']);exit;
            $this->load->view('include/header');
            $this->load->view('frontoffice/viewBusinessCenterRequest', $business_center_reservation);
            $this->load->view('include/footer');
        }
    }
    public function business_req_autoload()
    {
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        ...
        $busn_center_rsv['list'] = $this->FrontofficeModel->get_business_center_reservation_list_app($admin_id);

        print_r($busn_center_rsv['list']);exit;
        $this->load->view('frontoffice/autoload_BusinessCenterRequest', $busn_center_rsv);
    }
    ...
    public function enquiry()
    {
        ...
        if (!empty($check_in_date) && !empty($check_out_date)) {
            ...
            $data['list'] = $this->FrontofficeModel->search_en_request_data($check_in_date, $admin_id, $check_out_date);
            $data["list1"] = $this->FrontofficeModel->get_enquiry_accepted_by_user($admin_id);
            $data["list2"] = $this->FrontofficeModel->get_enquiry_rejected_by_user($admin_id);
            $this->load->view('include/header');
            $this->load->view('frontoffice/viewEnquirySearch', $data);
            $this->load->view('include/footer');
        } elseif (!empty($check_in_date) && empty($check_out_date)) {
            $data['list'] = $this->FrontofficeModel->ser_check_in_date_data($check_in_date, $admin_id);
            $data["list1"] = $this->FrontofficeModel->get_enquiry_accepted_by_user($admin_id);
            $data["list2"] = $this->FrontofficeModel->get_enquiry_rejected_by_user($admin_id);
            $this->load->view('include/header');
            $this->load->view('frontoffice/viewEnquirySearch', $data);
            ...
            $data['list'] = $this->FrontofficeModel->ser_check_out_date_data($check_out_date, $admin_id);
            $data["list1"] = $this->FrontofficeModel->get_enquiry_accepted_by_user($admin_id);
            $data["list2"] = $this->FrontofficeModel->get_enquiry_rejected_by_user($admin_id);
            $this->load->view('include/header');
            $this->load->view('frontoffice/viewEnquirySearch', $data);
            ...
            $enquiry_request["list1"] = $this->FrontofficeModel->get_enquiry_accepted_by_user($admin_id);
            $enquiry_request["list2"] = $this->FrontofficeModel->get_enquiry_rejected_by_user($admin_id);
            $this->load->view('include/header');
    ...
    public function reject_enquiry_request()
    {
        ...
        $where_u_id = '(hotel_enquiry_request_id = "' . $hotel_enquiry_request_id . '")';
        $enquiry = $this->MainModel->getData($tbl = 'hotel_enquiry_request', $where_u_id);
        $enquiry_u_id = $enquiry['u_id'];
        // print_r($enquiry_u_id);exit;
        ...
        if ($up) {
            ...
            if (!empty($get_dt)) {
                $deviceToken = $get_dt['token'];
                $title = 'Your Enquiry Request Has Been Rejected';
                $body = 'Your Enquiry Request ID is "' . $hotel_enquiry_request_id . '"';
                $result = send_push_notification($deviceToken, $title, $body);
            }
            // inside app notification
            $subject = $title;
            $description = "$title In $hotel_name And Your enquiry Id Is $hotel_enquiry_request_id";
            $arr_noti = array(
                'hotel_id' => $admin_id,
                'u_id' => $enquiry_u_id,
                'subject' => $subject,
    ...
    public function roomStatus()
    {
        ...
        // echo "<pre>";
        // print_r($room_status['floor_list']);
        // exit;
        $this->load->view('include/header');
        $this->load->view('frontoffice/viewRoomStatus', $room_status);
        // $this->load->view('frontoffice/viewRoomStatusDatewise',$room_status);
        $this->load->view('include/footer');
    }
    public function guests()
    {
        ...
        $check_in = $this->input->post('check_in');
			...
			if(!empty($check_in) && !empty($u_id))
				{

				   $guest_mng['list'] = $this->FrontofficeModel->search_guest_data($check_in,$admin_id,$u_id);
				//  echo "<pre>"; print_r($data['list']); echo "</pre>";die;
        ...
        $guest_mng['list1'] = $this->FrontofficeModel->get_departure_guest_list_pagination($admin_id);

        $guest_mng['list2'] = $this->FrontofficeModel->get_upcoming_guest_list_pagination($admin_id);
    }
        $this->load->view('include/header');
    ...
    public function guest_history1()
    {
        ...
        if ($this->session->userdata('u_id')) {
            ...
            $u_id1 = $this->uri->segment(4);

            $guest_history['user_data'] = $this->FrontofficeModel->get_user_data($u_id1);
            $guest_history['booking_details'] = $this->FrontofficeModel->get_user_booking_details($admin_id, $booking_id);
            $guest_history['booking_history'] = $this->FrontofficeModel->get_user_booking_history($admin_id, $u_id1);
            $guest_history['room_number'] = $this->FrontofficeModel->get_booking_room_details($booking_id);

            $this->load->view('include/header');
            $this->load->view('frontoffice/viewGuestHistory', $guest_history);
            $this->load->view('include/footer');
        } else {
            redirect(base_url());
        }
    ...
    public function guest_history()
    {
        ...
        $guest_history['booking_details'] = $this->FrontofficeModel->get_user_booking_details($admin_id, $booking_id);
        $guest_history['booking_history'] = $this->FrontofficeModel->get_user_booking_history($admin_id, $u_id1);
        // print_r($guest_history['booking_history']);exit;
        $guest_history['room_number'] = $this->FrontofficeModel->get_booking_room_details($booking_id);
    ...
    public function visitors()
    {
        ...
        $visit_date = $this->input->post('check_in');

        if (!empty($visit_date)) {
            $add_visitors['list'] = $this->FrontofficeModel->search_visitor_data($admin_id, $visit_date);

            $this->load->view('include/header');
            $this->load->view('frontoffice/searchvisitors', $add_visitors);
            $this->load->view('include/footer');

        } else {
            $this->load->view('include/header');
            $this->load->view('frontoffice/viewVisitors');
            $this->load->view('include/footer');
        }
    }

    public function get_visiters_data()
    {
        ...
        $visiter_data = $this->FrontofficeModel->get_visiter_data($rowperpage, $row, $search, $columnName, $hotel_id);
        $total_get_visiter_data = $this->FrontofficeModel->get_total_visiter_data($search, $hotel_id);
        ...
        foreach ($visiter_data as $val) {
            ...
            if ($val['is_otp_verified'] == 0) {
                $otpaction = '<a href="#" class="btn btn-secondary shadow btn-xs open_model otp_lock_btn" id = "edit_data" data-id1="' . $val['visitor_id'] . '"><i class="fa fa-unlock-alt text-white"></i></a>';
            } else {
                if ($val['is_otp_verified'] == 1 && $val['is_otp_correct'] == 1) {
                    $otpaction = '<span class="badge badge-success">Success</span>';
                } else {
                    if ($val['is_otp_verified'] == 2 && $val['is_otp_correct'] == 2) {
                        $otpaction = '<span class="badge badge-danger open_model otp_fail_btn" id="edit_data" data-id1="' . $val['visitor_id'] . '">Fail</span>';
                    }
                }
            }
            ...
            $data[] = array(
                ...
                "No_Of_Visitor" => $val['no_of_visitor'],
                "Visiting_Date" => date("d-m-Y", strtotime($val['visit_date'])),
                "Visiting_Time" => $Visiting_Time,
                "Contact_No" => $val['mobile_no'],
    ...
    public function get_business_data()
    {
        ...
        $business_data = $this->FrontofficeModel->get_business_data($rowperpage, $row, $search, $columnName, $hotel_id);

        $total_get_business_data = $this->FrontofficeModel->get_total_business_data($search, $hotel_id);
        $totalRecords = $total_get_business_data->total_record;
        // print_r($totalRecords);exit;

        $data = array();
        $i = 1;
        foreach ($business_data as $val) {
            $wh = '(business_center_id = "' . $val['business_center_type'] . '")';

            $business_center_type = $this->FrontofficeModel->getData('business_center', $wh);
            // print_r($business_center_type);exit;
            $date1 = date('d-m-Y', strtotime($val['created_at']));
            $date = '<h5>' . $date1 . '</h5>';

            $start_time1 = date('h:i A', strtotime($val['start_time']));
            $start_time = '<h5>' . $start_time1 . '</h5>';

            $end_time1 = date('h:i A', strtotime($val['end_time']));
            $end_time = '<h5>' . $end_time1 . '</h5>';

            $data[] = array(
                "sr_no" => $row + $i++,
                "client_name" => $val['client_name'],
                "business_center_type" => isset($business_center_type['business_center_type']) ? $business_center_type['business_center_type'] : "-",
                "no_of_people" => isset($business_center_type['no_of_people']) ? $business_center_type['no_of_people'] : "-",
                "date" => $date,
                "start_time" => $start_time,
                "end_time" => $end_time,
                "status" => '<a href="#"  class="disabled_data btn btn-warning shadow btn-xs sharp me-1" data-bs-toggle="modal" id="edit_data" data-id="' . $val['b_c_reserve_id'] . '" data-bs-target=".update_staff_modal"><i class="fa fa-share"></i></a>',
                "view" => '<a href="#"  class="btn btn-secondary shadow btn-xs sharp me-1" data-bs-toggle="modal" id="edit_data"  data-id="' . $val['b_c_reserve_id'] . '" data-bs-target=".bd-note-modal-lg"><i class="fa fa-eye"></i></a>',
                "created_at" => $val['created_at'],
            );
			if($business_center_type != NULL)
			{
				$data['status'] = '<a href="#"  class="btn btn-warning shadow btn-xs sharp me-1" data-bs-toggle="modal" id="edit_data" data-id="' . $val['b_c_reserve_id'] . '" data-bs-target=".update_staff_modal"><i class="fa fa-share"></i></a>';
			}
    ...
    public function auto_load_visiters_data()
    {
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        ...
        $this->load->view('frontoffice/auto_load_Visitors', $add_visitors);
    }
    ...
    public function notification()
    {
        ...
        if (!empty($date)) {
            ...
            $send_department['list'] = $this->FrontofficeModel->get_admin_sent_user_notifications_search($admin_id, $date);
            // print_r($send_department['list']);die;
    ...
    public function delete_notification()
    {
        ...
        $del = $this->FrontofficeModel->delete_data('notifications', $wh);
        ...
        if ($del) {
            ...
            $send_department['guest_list'] = $this->FrontofficeModel->get_guest_list_by_hotels($admin_id);
            $send_department['list'] = $this->FrontofficeModel->get_admin_sent_user_notifications($admin_id);
// print_r($send_department['list']);exit;
            // $this->load->view('include/header');
    ...
    public function delete_room_config()
    {
        $u_id = $this->session->userdata('u_id');
        $wh_admin = '(u_id ="' . $u_id . '")';
        $get_hotel_id = $this->MainModel->getData('register', $wh_admin);
        ...
        $room_configure_id = $this->uri->segment(3);
        ...
        foreach ($old_room_no as $o_rn) {
            ...
            $room_status = $this->FrontofficeModel->getData('room_status', $wh1);
        ...
        if (count($deleted_rooms) === count($old_room_no)) {
            ...
            $del = $this->FrontofficeModel->delete_data('room_configure', $wh);
            $del = $this->FrontofficeModel->delete_data('room_facility', $wh);
            $del = $this->FrontofficeModel->delete_data('room_imgs', $wh);
        }
        ...
        foreach ($deleted_rooms as $room_data) {
            ...
            $del = $this->FrontofficeModel->delete_data('room_status', $wh1);
            $del = $this->FrontofficeModel->delete_data('room_nos', $wh1);
        }
    ...
    public function view_config()
    {
        ...
        $view_room['floor_list'] = $this->FrontofficeModel->get_floor_list($admin_id);
        $this->load->view('include/header');
        $this->load->view('frontoffice/view_room_config', $view_room);
        $this->load->view('include/footer');
    ...
    public function serviceRequest()
    {
        ...
        $service_request['list'] = $this->FrontofficeModel->get_service_request($u_id, $todays_date);
        $service_request['list2'] = $this->FrontofficeModel->get_hotel_service_request($admin_id, $todays_date);
        $service_request['list3'] = $this->FrontofficeModel->get_accept_service_request($admin_id);
        $service_request['list4'] = $this->FrontofficeModel->get_reject_service_request($admin_id);
        // print_r($service_request['list1']);exit;
        $this->load->view('include/header');
    ...
    public function ajaxSubIconBlockView()
    {
        ...
        if ($postArr['data_id'] == "10") {
            if ($postArr['sub_section_id'] == "1") {
                ...
                $guest_mng["vehicle_wash_request7"] = $this->FrontofficeModel->get_vehicle_wash_completed_request_list_pagination($admin_id);
                $guest_mng["vehicle_wash_request"] = $this->FrontofficeModel->get_vehicle_wash_request_list_pagination($admin_id);
                $guest_mng["vehicle_wash_request1"] = $this->FrontofficeModel->get_vehicle_wash_accepted_request_list_pagination($admin_id);
                $guest_mng["vehicle_wash_request3"] = $this->FrontofficeModel->get_vehicle_wash_rejected_request_list_pagination($admin_id);
                $guest_mng["vehicle_wash_request2"] = $this->FrontofficeModel->get_vehicle_wash_charges_list_pagination($admin_id);
            } else if ($postArr['sub_section_id'] == "7") {
                $guest_mng['list_charges'] = $this->FrontofficeModel->get_luggage_charges_list_pagination($admin_id);
                $guest_mng["luggage_request"] = $this->FrontofficeModel->get_luggage_request_list_pagination($admin_id);
                $guest_mng["luggage_request1"] = $this->FrontofficeModel->get_luggage_accepted_request_list_pagination($admin_id); // print_r($guest_mng["luggage_request"]);exit;
                $guest_mng["luggage_request2"] = $this->FrontofficeModel->get_luggage_rejected_request_list_pagination($admin_id);
                $guest_mng["luggage_request4"] = $this->FrontofficeModel->get_luggage_completed_request_list_pagination($admin_id);
                // print_r($guest_mng["luggage_request4"]);exit;
            } else if ($postArr['sub_section_id'] == "8") {
                $guest_mng["list"] = $this->FrontofficeModel->get_cab_service_pagination($admin_id);
                $guest_mng["list1"] = $this->FrontofficeModel->cab_service_completed_pagination($admin_id);
                $guest_mng["list2"] = $this->FrontofficeModel->cab_service_rejected_pagination($admin_id);
                $guest_mng["list3"] = $this->FrontofficeModel->cab_service_completed_tab($admin_id);
                $guest_mng["list4"] = $this->FrontofficeModel->cab_service_cancle_tab($admin_id);
            }
        }
        $this->load->view('frontoffice/ajaxFrontSubIconBlockView', $guest_mng);
    }

    public function auto_load_service_req_data()
    {
        $postArr = $this->input->post();
        // echo "<pre>";
        // print_r($postArr);
        ...
        if ($postArr['sub_section_id'] == "2") {
            $guest_mng["spa_request"] = $this->FrontofficeModel->get_spa_request_list_pagination($admin_id);
            $this->load->view('frontoffice/spa_req_autoload', $guest_mng);
        }
        if ($postArr['sub_section_id'] == "6") {
            $guest_mng["vehicle_wash_request"] = $this->FrontofficeModel->get_vehicle_wash_request_list_pagination($admin_id);
        }
        if ($postArr['sub_section_id'] == "7") {
            $guest_mng["luggage_request"] = $this->FrontofficeModel->get_luggage_request_list_pagination($admin_id);
        }
        if ($postArr['sub_section_id'] == "8") {
            $guest_mng["list"] = $this->FrontofficeModel->get_cab_service_pagination($admin_id);
        }
    }
    public function getrequirement_car()
    {
        ...
        // $wh_l = '(food_order_id = "' . $id . '")';
        //     $data = $this->MainModel->getAllData1('food_order_details', $wh_l);
        $guest_mng["vehicle_wash_request"] = $this->FrontofficeModel->get_car_request_list1($hotel_id, $id);
        // echo "<pre>";print_r($guest_mng["vehicle_wash_request"]);exit;
    ...
    public function change_new_car_status()
    {
        ...
        // print_r($vehicle_wash_request_id);exit;
        $request_status = $this->input->post('request_status');
        $reject_reason = $this->input->post('reject_reason');
        $vehivle_u_id = $this->input->post('car_u_id');
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        $admin_id = $admin_details['hotel_id'];
        ...
        if ($request_status == 1) {
            ...
            $up = $this->FrontofficeModel->edit_data('vehicle_wash_request', $wh, $arr);
            ...
            if ($up) {
                ...
                $subject = $title;
                $description = "$title In $hotel_name And Your Vehicle Wash Service Id Is $vehicle_wash_request_id";
                $arr_noti = array(
                    'hotel_id' => $admin_id,
                    'u_id' => $vehivle_u_id,
                    'subject' => $subject,
                ...
                $this->load->view('frontoffice/ajaxcarwashrequest', $guest_mng);
            } else {
                $this->session->set_flashdata('msg', 'Something went wrong');
                redirect(base_url('FrontofficeController/viewServiceBooking'));
            }
        } elseif ($request_status == 2) {
            $arr1 = array(
                'request_status' => $request_status,
            ...
            $up = $this->FrontofficeModel->edit_data('vehicle_wash_request', $wh1, $arr1);
            ...
            if ($up) {
                ...
                if ($get_u_id['reject_reason'] == 1) {
                    ...
                    $rejct_reason = "Vehicle Not Identified";
                } else if ($get_u_id['reject_reason'] == 4) {
                    $rejct_reason = "Please contact Front office";
                } else {
                    $rejct_reason = "";
                ...
                if ($get_dt) {
                    ...
                    $body = 'Your Vehicle Wash Request ID is "' . $vehicle_wash_request_id . '" And Your Request is Rejected Because OF "' . $rejct_reason . '"';
                    $result = send_push_notification($deviceToken, $title, $body);
                }
                //    $this->session->set_flashdata('msg','Request rejected successfully');
                $guest_mng["vehicle_wash_request"] = $this->FrontofficeModel->get_vehicle_wash_request_list_pagination($admin_id);
                $subject = $title;
                $description = "$title In $hotel_name And Your Vehicle Wash Service Id Is $vehicle_wash_request_id And Your Request is Rejected Because OF $rejct_reason ";
                $arr_noti = array(
                    'hotel_id' => $admin_id,
                    'u_id' => $vehivle_u_id,
                    'subject' => $subject,
    ...
    public function luggage_status()
    {
        ...
        $hotel_name = $get_hotel_name['hotel_name'];
        $arr = array(
            'is_completed' => $_POST['status'],
            'complete_date' => $today_date,
        );

        $u_id = $_POST['uid'];
        $where = '(luggage_request_id="' . $u_id . '")';
        $id = $this->FrontofficeModel->editData($tbl = "luggage_request", $where, $arr);
        $get_u_id1 = $this->MainModel->getData($tbl = 'luggage_request', $where);
        // $wh1 = '(u_id = "'.$get_u_id['u_id'].'")';
        $u_id_luggage = $get_u_id1['u_id'];
        $hotel_id_luggage = $get_u_id1['hotel_id'];
        if ($id) {
            ...
            if (!empty($get_dt)) {
                $deviceToken = $get_dt['token'];
                $title = 'Luggage Is Return';
                $body = 'Your Luggage Request ID is "' . $u_id . '"';
                $result = send_push_notification($deviceToken, $title, $body);
            }
            $subject = $title;
            $description = "$title In $hotel_name And Your Completed Vehicle Service Id Is $u_id";
            $arr_noti = array(
                'hotel_id' => $hotel_id_luggage,
                'u_id' => $u_id_luggage,
                'subject' => $subject,
                'description' => $description,
                'clear_flag' => 1,
    ...
    public function ajaxroom_types()
    {
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
    ...
    public function ajaxOrderIconView_lost()
    {
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        ...
        $lost_found["completed"] = $this->FrontofficeModel->completed_lost_found_pagination($admin_id);
        // print_r($lost_found["completed"]);die;
    ...
    public function ajaxOrderIconView_v2()
    {
        ...
        $guest_mng["spa_request2"] = $this->FrontofficeModel->get_spa_reject_list_pagination($admin_id);
        $guest_mng["spa_request6"] = $this->FrontofficeModel->get_spa_complete_list_pagination($admin_id);
        // print_r($guest_mng["spa_request6"]);exit;
        $guest_mng["list"] = $this->FrontofficeModel->get_front_ofs_services_pagination($admin_id, $service_name);
        $this->load->view('frontoffice/ajaxFrontSubIconBlockView', $guest_mng);
    ...
    public function cab_status()
    {
        ...
        $u_id = $_POST['uid'];
        $where = '(cab_service_request_id="' . $u_id . '")';
        $id = $this->FrontofficeModel->editData($tbl = "cab_service_request_list", $where, $arr);
        $get_u_id1 = $this->MainModel->getData($tbl = 'cab_service_request_list', $where);
        // $wh1 = '(u_id = "'.$get_u_id['u_id'].'")';
        $u_id_cab = $get_u_id1['u_id'];
        $hotel_id_cab = $get_u_id1['hotel_id'];
        if ($id) {
            ...
            if (!empty($get_dt)) {
                ...
                $body = 'Your Cab Service Request ID is "' . $u_id . '"';
                $result = send_push_notification($deviceToken, $title, $body);
            }
            $subject = $title;
            $description = "$title In $hotel_name And Your Completed Cab Service Id Is $u_id";
            $arr_noti = array(
                'hotel_id' => $hotel_id_cab,
                'u_id' => $u_id_cab,
                'subject' => $subject,
                'description' => $description,
                'clear_flag' => 1,
    ...
    public function carwash_status()
    {
        ...
        $u_id = $_POST['uid'];
        $where = '(vehicle_wash_request_id="' . $u_id . '")';
        $id = $this->FrontofficeModel->editData($tbl = "vehicle_wash_request", $where, $arr);
        $get_u_id1 = $this->MainModel->getData($tbl = 'vehicle_wash_request', $where);
        $u_id_car = $get_u_id1['u_id'];
        $hotel_id_car = $get_u_id1['hotel_id'];
        if ($id) {
            ...
            if (!empty($get_dt)) {
                ...
                $body = 'Your Vehicle Wash Request ID is "' . $u_id . '"';
                $result = send_push_notification($deviceToken, $title, $body);
            }
            $subject = $title;
            $description = "$title In $hotel_name And Your Completed Vehicle Service Id Is $u_id";
            $arr_noti = array(
                'hotel_id' => $hotel_id_car,
                'u_id' => $u_id_car,
                'subject' => $subject,
                'description' => $description,
                'clear_flag' => 1,
    ...
    public function ajaxOrderIconView_v4()
    {
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
    ...
    public function event_active_deactive()
    {
        $postArr = $this->input->post();
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        ...
        $hotel_id = $admin_details['hotel_id'];
        $id = $postArr['id'];
        $event_status = $postArr['event_status'];
        $active_deactive_subid = $postArr['active_deactive_subid'];
        $wh = '(front_ofs_service_id = "' . $id . '" AND service_name = "' . $active_deactive_subid . '" AND hotel_id = "' . $hotel_id . '")';
        $arr = array(
            'status' => $event_status,
        );
        $data = $this->FrontofficeModel->edit_data('front_ofs_services', $wh, $arr);
        // echo"<pre>";
        // print_r($id);
        // exit;
    }
    ...
    public function delete_vehicle_washing_charges()
    {
        ...
        // print_r($vehicle_washing_charge_id);exit;
        $wh = '(vehicle_washing_charge_id = "' . $vehicle_washing_charge_id . '")';

        $del = $this->FrontofficeModel->delete_data('vehicle_washing_charges', $wh);
    ...
    public function edit_vehicle_washing_charges()
    {
        ...
        $up = $this->FrontofficeModel->edit_data('vehicle_washing_charges', $wh, $arr);

        if ($add) {
            $guest_mng["vehicle_wash_request2"] = $this->FrontofficeModel->get_vehicle_wash_charges_list_pagination($admin_id);
            $this->load->view('frontoffice/ajaxcarwashcharges', $guest_mng);
        } else {
            $this->session->set_flashdata('msg', 'Something went wrong');
            redirect(base_url('serviceBooking'));
        }
    ...
    public function add_vehicle_washing_charges()
    {
        ...
        $vehicle_washing_charges_data = $this->FrontofficeModel->getData('vehicle_washing_charges', $wh);
        ...
        if ($vehicle_washing_charges_data) {
            ...
            $add = $this->FrontofficeModel->insert_data('vehicle_washing_charges', $arr);
    ...
    public function edit_luggage_charges()
    {
        ...
        $up = $this->FrontofficeModel->edit_data('luggage_charges', $wh, $arr);
    ...
    public function shuttle_Avil()
    {
        ...
        $guest_mng['mon_list'] = $this->FrontofficeModel->get_shuttle_service_staff_avaibility_by_day($admin_id, $day2);
// print_r($guest_mng['mon_list']);exit;
        $guest_mng['tue_list'] = $this->FrontofficeModel->get_shuttle_service_staff_avaibility_by_day($admin_id, $day3);
...
// print_r($guest_mng['mon_list']);exit;
            ...
            $this->load->view('frontoffice/ajaxFrontSubIconBlockView', array(
                'list' => $guest_mng["list"],
    ...
    public function change_available_status()
    {
        ...
        $update = $this->FrontofficeModel->edit_data('shuttle_service_avaibility', $wh, $arr);
...
// print_r($guest_mng['mon_list']);exit;
    ...
    public function staffhandover()
    {
        if ($this->session->userdata('u_id')) {
            ...
            $admin_id = $admin_details['hotel_id'];
            $today_date = date('Y-m-d');
            $handover["staff_pending_list"] = $this->FrontofficeModel->get_staff_handover_pagination($admin_id, 0, $today_date);
            $handover["staff_complete_list"] = $this->FrontofficeModel->get_staff_complete_handover_pagination($admin_id, 1);

            $this->load->view('include/header');
            $this->load->view('frontoffice/viewStaffHandover', $handover);
            $this->load->view('include/footer');
        } else {
            redirect(base_url());
        }
    ...
    public function add_guest_count()
    {
        ...
        $output .= (int) $adult + (int) $child;
        // print_r($output);exit;
        echo $output;
    }
    public function ajax_floors()
    {
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
    ...
    public function getStaffData()
    {
        ...
        $data = $this->FrontofficeModel->getregdata($tbl = 'business_center_reservation', $wh);
        // echo "<pre>";print_r($data);die;
        echo json_encode($data);
    ...
    public function serviceId()
    {
        ...
        $data = $this->FrontofficeModel->getServiceId($tbl = 'service_request', $wh);
        // echo "<pre>";print_r($data);die;
        echo json_encode($data);
    ...
    public function enquiryId()
    {
        ...
        $data = $this->FrontofficeModel->getEnquiryId($tbl = 'hotel_enquiry_request', $wh);
        // echo "<pre>";print_r($data);die;
        echo json_encode($data);
    ...
    public function change_service_status()
    {
        ...
        if ($request_status == 1) {
            ...
            $where = '(service_request_id ="' . $u_id . '")';
            $where2 = '(ser_id ="' . $u_id . '")';
            $add = $this->MainModel->edit_data($tbl = "service_request", $where, $arr);
            $add = $this->MainModel->edit_data($tbl = "notifications", $where2, $arr2);
            // print_r($where);exit;
            if ($add) {
                $u_id = $this->session->userdata('u_id');
                $where = '(u_id = "' . $u_id . '")';
            ...
            $where1 = '(service_request_id ="' . $u_id . '")';
            $where2 = '(ser_id ="' . $u_id . '")';
            $add = $this->MainModel->edit_data($tbl = "service_request", $where1, $arr1);
            $add = $this->MainModel->edit_data($tbl = "notifications", $where2, $arr2);
            if ($add) {
                $u_id = $this->session->userdata('u_id');
                $where = '(u_id = "' . $u_id . '")';
    ...
    public function change_user_status()
    {
        ...
        if ($request_status == 1) {
            ...
            $add = $this->MainModel->edit_data($tbl = "business_center_reservation", $where, $arr);
            // print_r($where);exit;
            if ($add) {
                ...
                if (!empty($get_dt)) {
                    ...
                    $body = 'Your Business Center Request ID is "' . $u_id . '"';
                    $result = send_push_notification($deviceToken, $title, $body);
                }
                ...
                $this->session->set_flashdata('add', 'Something went Wrong');
                redirect(base_url('businessCenterRequest'));
            }
        } elseif ($request_status == 2) {
            // $a=$this->input->post();print_r($a);exit;
            $arr1 = array(
            ...
            $where1 = '(b_c_reserve_id ="' . $u_id . '")';
            $add = $this->MainModel->edit_data($tbl = "business_center_reservation", $where1, $arr1);
            if ($add) {
                ...
                if (!empty($get_dt)) {
                    ...
                    $body = 'Your Business Center Request ID is "' . $u_id . '" And Your Request is Rejected Because OF "' . $reject_reason . '" ';
                    $result = send_push_notification($deviceToken, $title, $body);
                }
        ...
        if ($up) {
            ...
            if (isset($_FILES['image']['name'])) {
                ...
                for ($k = 0; $k < $count1; $k++) {
                    ...
                    $room_img = $this->MainModel->getData('room_imgs', $wh_rm_i);
                    ...
                    $this->FrontofficeModel->edit_data('room_imgs', $wh_rm_i, $arr_rm_i);
                }
            }
    ...
    public function get_user_id()
    {
        ...
        $guest_data = $this->FrontofficeModel->get_room_no_data1($hotel_id, $room_no, $todays_date);
        if (!empty($guest_data)) {
            $output = '';
    ...
    public function add_expenses()
    {
        ...
        if ($user_data) {
            ...
            if ($add) {
                ...
                $data["list"] = $this->FrontofficeModel->get_user_expense_pagination($admin_id);
                //  print_r($data["list"]);exit;
                $this->session->set_flashdata('msg', 'Data Updated Successfully');
                $this->load->view('frontoffice/ajaxviewExpense', $data);
            }
        }
    ...
    public function add_booking_id()
    {
        ...
        $guest_user = $this->MainModel->getData_booking('guest_user', $wh1);
    ...
    public function assign_rooms()
    {
        ...
        $booking_data = $this->MainModel->getData('user_hotel_booking', $wh_detail);
        $assign_u_id = $booking_data['u_id'];
        //   print_r($assign_u_id);exit;
        //      $where_u_id ='(hotel_enquiry_request_id = "'.$hotel_enquiry_request_id.'")';
        //  $enquiry = $this->MainModel->getData($tbl ='hotel_enquiry_request', $where_u_id);
        // $enquiry_u_id = $enquiry['u_id'];
        $add_details = "";
    ...
    public function update_pending_handover()
    {
        ...
        if ($result) {
            ...
            $data['pending_handover'] = $this->FrontofficeModel->get_manager_handover_list_pagination($hotel_id);
            // print_r($data['pending_handover']);
            $this->load->view('frontoffice/ajaxviewHandover', $data);
        } else {
            $this->session->set_flashdata('pending_handover_error_msg', 'Something Went Wrong..!');
            redirect(base_url() . 'handover');
    ...
    public function add_carwash_request()
    {
        ...
        $whs = '(booking_id = "' . $booking_id . '")';
        $get_u_id = $this->ApiModel->getData('user_hotel_booking', $whs);
        ...
        $up = $this->FrontofficeModel->insert_data('vehicle_wash_request', $arr);
        if ($up) {
    ...
    public function add_cab_request()
    {
        ...
        if ($admin_details) {
            ...
            $add = $this->FrontofficeModel->insert_data('cab_service_request_list', $arr);
            ...
            if ($add) {
                ...
                $guest_mng["list"] = $this->FrontofficeModel->get_cab_service_pagination($admin_id);
                // $this->session->set_flashdata('msg','Data Updated Successfully');
                $this->load->view('frontoffice/ajaxcabservice', $guest_mng);
            }
        }
    }
    public function add_luggage_request()
    {
        ...
        if ($admin_details) {
            ...
            //  print_r($arr);exit;
            $up = $this->FrontofficeModel->insert_data('luggage_request', $arr);
            // print_r($up);exit;
            if ($up) {
    ...
    public function add_spa_request()
    {
        ...
        if ($admin_details) {
            $arr = array(
                ...
                'charges' => $charges,
                'request_date' => date('Y-m-d'),
                'request_time' => date('H:i:s', strtotime('now')),
                'note' => $additional_note,
            );
            //  print_r($arr);exit;
            $up = $this->FrontofficeModel->insert_data('spa_service_request_list', $arr);
    ...
    public function update_room_type()
    {
        ...
        if ($room_type_data) {
            ...
            if ($add) {
                ...
                $room_configuration_data = $this->FrontofficeModel->getData('room_configure', $wh1);

                if ($room_configuration_data) {
                    $arr1 = array(
                        'price' => $price,
                    );
                    $edit_data = $this->FrontofficeModel->edit_data('room_configure', $wh1, $arr1);
    ...
    public function geteditbabyData()
    {
        ...
        $guest_mng = $this->FrontofficeModel->getbabysingledata($tbl = 'front_ofs_services', $where);
    ...
    public function geteditpoolData()
    {
        ...
        // print_r($wh);die;
        $wh = '(front_ofs_service_id ="' . $where . '")';
        $guest_mng = $this->FrontofficeModel->getpoolsingledata($tbl = 'front_ofs_services', $where);
    ...
    public function geteditspatype()
    {
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        ...
        $service_name = 2;
        $wh = '(front_ofs_service_id ="' . $where . '")';
        $guest_mng = $this->FrontofficeModel->get_spa_services_images($admin_id, $where);
        //  print_r($guest_mng);exit;
        echo json_encode($guest_mng);
            ...
            if ($service_name == 4) {
                if (isset($_FILES['shuttle_img']['name'])) {
                    if (!empty($_FILES['shuttle_img']['name'])) {
                        ...
                        $this->load->library('upload', $config);
                        $this->upload->initialize($config);
                        if ($this->upload->do_upload('my_shuttle_file')) {
                            $file_data = $this->upload->data();

                            $shuttle_img = base_url() . $path . $file_data['file_name'];
// print_r($shuttle_img);exit;
            ...
            if ($service_name == 1) {
                ...
                $admin_id = $admin_details['hotel_id'];
                $guest_mng["list"] = $this->FrontofficeModel->get_front_ofs_services_pagination($admin_id, $service_name);
                // print_r($guest_mng["list"]);exit;
                $this->load->view('frontoffice/ajaxeditgymcenter', $guest_mng);
            } else {
                if ($service_name == 2) {
                    $u_id = $this->session->userdata('u_id');
                    $where = '(u_id = "' . $u_id . '")';
                    ...
                    $this->load->view('frontoffice/ajaxsparequest_edit', $guest_mng);
                } else {
                    if ($service_name == 3) {
                        $u_id = $this->session->userdata('u_id');
                        $where = '(u_id = "' . $u_id . '")';
                        ...
                        $this->load->view('frontoffice/ajaxeditpoolcenter', $guest_mng);
                    } else {
                        if ($service_name == 4) {
                            $u_id = $this->session->userdata('u_id');
                            $where = '(u_id = "' . $u_id . '")';
                            ...
                            $this->load->view('frontoffice/ajaxeditshuttlecenter', $guest_mng);
                        } else {
                            if ($service_name == 5) {
                                $u_id = $this->session->userdata('u_id');
                                $where = '(u_id = "' . $u_id . '")';
                                ...
                                $this->load->view('frontoffice/ajaxeditbabycenter', $guest_mng);
    ...
    public function geteditshuttleData()
    {
        ...
        // print_r($wh);die;
        $wh = '(front_ofs_service_id ="' . $where . '")';
        $guest_mng['data'] = $this->FrontofficeModel->getshuttlesingledata($tbl = 'front_ofs_services', $where);
        // $guest_mng['images'] = $this->FrontofficeModel->getshuttlesingledata($tbl ='front_ofs_services_images', $wh);
    ...
    public function getCabData()
    {
        $wh = $this->input->post('id');
        $data = $this->FrontofficeModel->getcabdata($tbl = 'cab_service_request_list', $wh);
        echo json_encode($data);
    }
    ...
    {
        $wh = $this->input->post('id');
        $data = $this->MainModel->getcarData($tbl = 'vehicle_wash_request', $wh);
        echo json_encode($data);
    }
    public function getLostData()
    {
        ...
        if ($data['lost_item_name'] != null) {
            // print_r('hello');exit;
            $data['item_name'] = $data['lost_item_name'];
        } elseif ($data['found_item_name'] != null) {
            $data['item_name'] = $data['found_item_name'];
        }
        // print_r($data['found_item_name']);exit;
        echo json_encode($data);
    }
    ...
    {
        $wh = $this->input->post('id');
        $data = $this->FrontofficeModel->getHandover($tbl = 'handover_manger', $wh);
        // print_r($data);exit;
        echo json_encode($data);
    }
    ...
    public function accept_cloakroom_service_request()
    {
        ...
        if ($up) {
            ...
            $title = 'Luggage Request Is Accepted';
            $body = array(
                'luggage_request_id' => $luggage_request_id,
            );
            $result = send_push_notification($deviceToken, $title, $body);
            // print_r($result);die;
...
// print_r($hotel_name);exit;
        ...
        if ($admin_wallet_points < 0 || $admin_wallet_points == 0) {
            ...
            if ($up) {
                ...
                if (!empty($get_dt)) {
                    $deviceToken = $get_dt['token'];
                    $title = 'Your Enquiry Request Has Been Accepted';
                    $body = 'Your Enquiry Request ID is "' . $hotel_enquiry_request_id . '" ';
                    $result = send_push_notification($deviceToken, $title, $body);
                }
    ...
    public function newenquiryRequest()
    {
        ...
        $enquiry_request["list"] = $this->FrontofficeModel->get_hotel_enquiry_request_pagination($hotel_id, $todays_date);
...
// print_r($arr);exit;
    ...
    public function check_visitor_otps()
    {
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        ...
        if ($data['otp'] == $otp) {
            ...
            if ($up) {
                ...
                if (!empty($get_dt)) {
                    ...
                    $body = 'Your Visitor Request ID is "' . $visitor_id . '"';
                    $result = send_push_notification($deviceToken, $title, $body);
                }

                $add_visitors['list'] = $this->FrontofficeModel->search_visitor_data($admin_id, $visit_date);
                $this->load->view('frontoffice/ajaxotpcheck', $add_visitors);
            ...
            if ($up) {
                ...
                if (!empty($get_dt)) {
                    ...
                    $body = 'Your Visitor Request ID is "' . $visitor_id . '" And Your Otp is Not Verified';
                    $result = send_push_notification($deviceToken, $title, $body);
                }
                echo 1;
                $add_visitors['list'] = $this->FrontofficeModel->search_visitor_data($admin_id, $visit_date);
    ...
    public function check_visitor_otp()
    {
        ...
        if ($data['otp'] == $otp) {
            ...
            if ($up) {
                ...
                $add_visitors["list"] = $this->FrontofficeModel->get_visitor_pagination($visitor_id);
                $this->load->view('frontoffice/ajaxotpcheck', $add_visitors);
                //  $this->session->set_flashdata('msg','OTP verified Successfully');
                //  redirect(base_url('visitors'));
            }
    ...
    public function checkout_bill()
    {
        ...
        if ($check_in == date('Y-m-d') && $is_today_charge == 1) {
            ...
            if ($up_ch) {
                ...
                $ck_request_status = $this->FrontofficeModel->edit_data('user_checkout_requests', $wh1, $arr_ckr);

                $wh_token = '(u_id ="' . $u_id . '")';
                $user_token_exist = $this->FrontofficeModel->getAllData('user_firebase_tokens', $wh_token);
                if ($user_token_exist) {
                    $arr_token = array(
                        'token' => "",
                    );

                    $reset_token = $this->FrontofficeModel->edit_data('user_firebase_tokens', $wh_token, $arr_token);
                }
                ...
                $clear_notification = $this->FrontofficeModel->edit_data('user_notification', $wh_n, $arr_n);
...
// print_r($booking_details_1);exit;
            ...
            if ($is_today_charge == 1) {
                $total = (int) $user_checkout_data['total_bill'] + (int) $amount;
                $is_today_charge = 1;
                $todays_charges = $amount;
            } else {
...
// print_r($business_center_reservation["list"]);exit;
    ...
    public function upcoming_guest()
    {
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        ...
        $upcoming_guest['list'] = $this->FrontofficeModel->get_upcoming_guest_list_pagination($admin_id);
        // print_r($upcoming_guest['list']);exit;
        $this->load->view('frontoffice/ajaxviewupcomingGuest', $upcoming_guest);
    }
    public function pendinghandover()
    {
        ...
        foreach ($pending_handover as $val) {
            ...
            $discription = '<a href="#" class="btn btn-secondary shadow btn-xs sharp me-1 description_modal" data-bs-toggle="modal" data-bs-target="#exampleModalCenter" data-id="' . $val['m_handover_id'] . '"><i class="fa fa-eye"></i></a>';
...
//    print_r($user_com);
    ...
    public function description_handover()
    {
        $id = $this->input->post('id');
        $u_id = $this->session->userdata('u_id');
        $where = '(u_id = "' . $u_id . '")';
        $admin_details = $this->MainModel->getData($tbl = 'register', $where);
        $hotel_id = $admin_details['hotel_id'];
        $data = $this->FrontofficeModel->get_pending_handover_modal($hotel_id, $id);
        // print_r($data["list"]);exit;
        echo json_encode($data);
    }
    ...
    // code by vivek
    public function frontDashnotification()
    {
        if ($this->session->userdata('u_id')) {
            $today_date = date('Y-m-d');
            ...
            // echo "<pre>";
            // print_r($all_notification['specific_hotel_notis_from_SA']);
            // exit;
            $this->load->view('include/header');
            $this->load->view('frontoffice/front_panel_notification', $all_notification);
            $this->load->view('include/footer');
        } else {
    ...
    public function gethandoverData()
    {
        ...
        $data = $this->FrontofficeModel->gethandata($tbl = 'lost_found_list', $wh);
        // echo "<pre>";print_r($data);die;
        echo json_encode($data);
    ...
    public function update_hand_sts()
    {
        ...
        $up = $this->FrontofficeModel->editData('lost_found_list', $wh, $arr);
        // print_r($up);
        // die;
        if ($up) {
            ...
            $this->session->set_flashdata('msg', 'Something went wrong');
            redirect(base_url('lost'));
        }
    }
    public function services_manage()
    {
        $table = 'hotels_services';
        $this->load->view('include/header');
        $this->load->view('frontoffice/view_service_manage');
        $this->load->view('include/footer');
    }

    public function get_hotel_all_services()
    {
        //  Start :: datatable code
        $draw = $this->input->post('draw');
        $row = $this->input->post('start');
        ...
        $all_hotel_services = $this->FrontofficeModel->get_all_hotels_services($rowperpage, $row, $search, $hotel_id);
        // echo "<pre>";
        // print_r($all_hotel_services);
        // exit;

        $total_all_hotel_services = $this->FrontofficeModel->getTotal_all_hotels_services($search, $hotel_id);
        $totalRecords = $total_all_hotel_services->total_record;

        $data = array();
        $i = 1;
        foreach ($all_hotel_services as $val) {
            $value = "";
            if ($val['status'] == 1) {
                $value = 'checked';
            } else {
            ...
            $action = '<label class="switchToggle">
			<input type="hidden" name="sub_icon_id" class="active_deactive_subid" value="2">
			<input type="checkbox" name="yes_no" class="yes_no" data-id="' . $val['hotels_service_id'] . '" ' . $value . '>
			<span class="slider green round"></span></label>';
            $data[] = array(
                "sr_no" => $row + $i++,
                "Service_Name" => $val['services_name'],
                "Action" => $action,
            );
        }
    ...
    public function hotel_service_status()
    {
        // echo "<pre>";
        // print_r($this->input->post());
        // exit;
        ...
        $hotels_service_id = $this->input->post('id');
        $service_status = $this->input->post('service_status');
        $where = '(hotels_service_id = "' . $hotels_service_id . '" AND hotel_id = "' . $hotel_id . '")';
        $arr = array(
            'status' => $service_status,
        );
        $service_status_change = $this->FrontofficeModel->editData('hotels_services', $where, $arr);
        echo json_encode($service_status_change);
        // echo "<pre>";
        // print_r($service_status_change);
        // exit;
    }
    public function checkOutRequest()
    {
        ...
        $data['list'] = $this->FrontofficeModel->get_all_checkout_request($admin_id);
        $data['list1'] = $this->FrontofficeModel->get_all_accepted_checkout_request($admin_id);
        $data['list2'] = $this->FrontofficeModel->get_all_completed_checkout_request($admin_id);

        //    echo "<pre>"; print_r($data['list2']); echo "</pre>";die;
        $this->load->view('include/header');
        $this->load->view('frontoffice/checkoutrequest', $data);
        $this->load->view('include/footer');
    }
    ...
    public function getotpdata()
    {
        ...
        $data = $this->FrontofficeModel->getdataotp($tbl = 'visitor', $wh);
    ...
    public function getrequirement()
    {
        ...
        // $wh_l = '(food_order_id = "' . $id . '")';
        //     $data = $this->MainModel->getAllData1('food_order_details', $wh_l);
        $guest_mng["spa_request"] = $this->FrontofficeModel->get_spa_request_list1($hotel_id, $id);
        // echo "<pre>";print_r($guest_mng["spa_request"]);exit;
