...
class HomeController extends CI_Controller {
	...
	{   
		...
		if($userType == 5){
			...
			if($this->session->userdata('u_id'))
			{
				...
				$front_dashboard['floor_list'] = $this->HotelAdminModel->get_floor_list($hotel_id);
		...
		if($userType == 7){
			...
			$hotel_id = $get_hotel_id['hotel_id']; 
			// floor list
			$food_dashboard['floor_list'] = $this->HotelAdminModel->get_floor_list($hotel_id);

			// today's count of table request
			$today_date = date('Y-m-d');
            $get_tab_reservation = $this->FoodAdminModel->get_count_of_tbl_req($admin_id,$today_date);
            $food_dashboard['count_tab_rev'] = $get_tab_reservation['total_tb_req'];
		...
		else if($userType == 2)
		{
			...
			// $front_dashboard['total_pending_booking'] = $this->HotelAdminModel->get_hotel_user_booking($admin_id,$booking_status1);
			...
			$front_dashboard['total_current_booking'] = $this->HotelAdminModel->get_hotel_current_booking($admin_id,$booking_status1);
			
			$front_dashboard['get_hotel_current_booking_accept'] = $this->HotelAdminModel->get_hotel_current_booking_accept($admin_id);
			
			$front_dashboard['total_pending_booking'] = $this->HotelAdminModel->get_hotel_expected_booking($admin_id,$booking_status1);
			
			$front_dashboard['total_expected_current_booking'] = $this->HotelAdminModel->get_hotel_expected_current__booking($admin_id);
			
			$front_dashboard['total_daparted_booking'] = $this->HotelAdminModel->get_hotel_depated_booking($admin_id,$booking_status3);
			
			$front_dashboard['total_expected_departed_booking'] = $this->HotelAdminModel->total_expected_departed_booking($admin_id);
			...
			$all_hotel_notis_from_SA = $this->FrontofficeModel->all_hotel_notic_from_superadmin($today_date);
			// echo "<pre>";print_r($all_hotel_notis_from_SA);die;

			$specific_hotel_notis_from_SA = $this->FrontofficeModel->specific_hotel_notis_from_SA($admin_id,$today_date);

			$get_front_ofs_notis = $this->FrontofficeModel->get_notifications_for_front_ofs($admin_id,$today_date);
		...
		else if($userType == 3)
		{
			...
			$front_dashboard['total_current_booking'] = $this->FrontofficeModel->get_hotel_current_booking($admin_id,$booking_status1);
			
			$front_dashboard['get_hotel_current_booking_accept'] = $this->FrontofficeModel->get_hotel_current_booking_accept($admin_id);
			
			$front_dashboard['total_pending_booking'] = $this->FrontofficeModel->get_hotel_expected_booking($admin_id,$booking_status1);
			
			$front_dashboard['total_expected_current_booking'] = $this->FrontofficeModel->get_hotel_expected_current_booking($admin_id);
			
			$front_dashboard['total_daparted_booking'] = $this->FrontofficeModel->get_hotel_depated_booking($admin_id,$booking_status3);
			
			$front_dashboard['total_expected_departed_booking'] = $this->FrontofficeModel->total_expected_departed_booking($admin_id);
			...
			$front_dashboard['get_front_ofs_notifications'] = array_merge($all_hotel_notis_from_SA,$specific_hotel_notis_from_SA,$get_front_ofs_notis);
			// echo "<pre>";print_r($front_dashboard['get_front_ofs_notifications']);die;
    ...
    public function food_bverages_dashboard()
	{
		if($this->session->userdata('u_id'))
		{
			...
			// today's count of table request
			$today_date = date('Y-m-d');
            $get_tab_reservation = $this->FoodAdminModel->get_count_of_tbl_req($hotel_id,$today_date);
            $food_dashboard['count_tab_rev'] = $get_tab_reservation['total_tb_req'];
			// echo "<pre>";
			// print_r( $food_dashboard['count_tab_rev']);
			// exit;
	...
	public function banq_hall_req_accept()
	{
		...
		if($up)
		{
			...
			if(!empty($get_dt)){
				...
				$body = 'Your Banquet Hall Request ID is "' . $banquet_hall_orders_id . '"';
    ...
    public function housekeeping_dashboard()
	{
		...
		if($this->session->userdata('u_id'))
		{
			...
			$housekeeping_dashboard['get_hk_notifications'] = $this->HotelAdminModel->get_notifications_for_housekeeping($admin_id,$today_date);
			// echo "<pre>";print_r($housekeeping_dashboard['get_hk_notifications']);die;
			$user_type = 9;
			$is_active1 = 1;
			$is_active2 = 0;
	...
	public function call_waiter_status()
	{
		
		$notifications= $this->input->post('id');
		$status = $this->input->post('status');
		if($status == 4)
		{
			$arr=array(
				'order_status'=> 4 //again incomplete
				);
		}
		if($status == 5)
		{
			$arr=array(
				'order_status'=> 5 //completed
				);
		}

		$where = '(notification_id="' . $notifications . '")';
		$id=$this->FrontofficeModel->editData($tbl="notifications",$where,$arr);
	...
	public function newManageOrder()
	{
		...
		if (isset($postArr['type']) && isset($postArr['food_facility_id']))
		 {	
			// echo "hi";die;
			$type = $postArr['type'];
			$food_facility_id = $postArr['food_facility_id'];

			$data["new_order_list"] = $this->FoodAdminModel->redirect_menu_new_order_list_pagination_food_admin($todays_date,$hotel_id,$food_facility_id);
				// echo "hi";die;
		// print_r($$data["new_order_list"]);exit;
			$data["completed_order"] = $this->FoodAdminModel->redirect_completed_menu_list_pagination($hotel_id,$food_facility_id);
			$data["rejected_order"] = $this->FoodAdminModel->redirect_rejected_menu_list_pagination($hotel_id,$food_facility_id);
		...
		else 
		{
			...
			$data["food_facility_id"]  = !empty($postArr['food_facility_id']) ? $postArr['food_facility_id'] : '';
	...
	public function new_food_ord_req_modal_data()
	{
		...
		$wh_l = '(food_order_id = "' . $food_order_id . '")';
	   	$data['get_food_orders_data'] = $this->FoodAdminModel->getAllData1('food_order_details', $wh_l);
	...
	public function get_out_of_time_orders()
	{
		...
		$time_out_ord = $this->FoodAdminModel->outof_time_food_order($ord_to_15min);
		$arr_to_string = array_column($time_out_ord,'food_order_id');
		$result =[];
		foreach($arr_to_string as $val_time)
		{
			$where = '(food_order_id = "'.$val_time.'" AND order_status = 0)';
			$arr= array(
	...
	public function out_of_time_food_orders_of_accepted()
	{
		log_message('DEBUG', 'acceptdetimeout_order--'.json_encode('newTime_out_color change_log'));
		$ord_to_15min = date('Y-m-d H:i:s', (time() - 60 * 14));
		$time_out_ord = $this->FoodAdminModel->outof_time_accepted_food_order($ord_to_15min);
		$arr_to_string = array_column($time_out_ord,'food_order_id');
		$result =[];
		foreach($arr_to_string as $val_time)
		{
			$where = '(food_order_id = "'.$val_time.'" AND order_status = 1)';
			$arr= array(
	...
	public function panel_notification()
	{
		if($this->session->userdata('u_id'))
		{
			...
			$all_room_notification['get_rs_notifications'] = $this->MainModel->get_notifications_for_rs_1($admin_id,$today_date);
// print_r($all_room_notification['get_rs_notifications']);die;
			$department_id = '';
			...
			$all_room_notification['get_staff_orders'] = $this->MainModel->get_staff_noti($admin_id,$department_id,$today_date);
			$this->load->view('include/header');
	...
	public function add_menu_orders()
	{
		...
		$food_facility_id = isset($_POST['food_facility_id']) && is_array($_POST['food_facility_id']) ? $_POST['food_facility_id'] : [];
		$food_facility_ids = implode(',',$food_facility_id);
		
		$total_menu_price = 0;
		$total_menu_prices_array = [];
		$total_menu_qty = 0;
		$count = count($food_menus_1);
		for ($i = 0; $i < $count; $i++) 
		{
			$quantitycount=$food_menu_qty[$i];
			// print_r($quantitycount);echo "hello";
			for ($j = 0; $j < $quantitycount; $j++) 
			{
				$total_menu_price = $total_menu_price + $food_menu_price[$i];

			}
			$total_menu_prices_array[] = $total_menu_price;

			// Reset the total_menu_price variable to 0 for the next iteration
			$total_menu_price = 0;
			// print_r($single_menu_total);
		}
		$totalcount = count($total_menu_prices_array);
		$order_total= 0 ;
		for ($k = 0; $k < $totalcount; $k++) 
		{
			$order_total = $order_total + $total_menu_prices_array[$k];
		}
			...
			if (!empty($food_menus_1)) 
			{
				...
				for ($i = 0; $i < $count; $i++) 
				{
					$arr3 = array(
									...
									'total' => $total_menu_prices_array[$i],
								);
	...
	{          
		...
		$result = $this->HotelAdminModel->delete_data('banquet_hall_images', $where);
		if($result)
		{
			$admin_id = $this->session->userdata('u_id');
			$wh_admin = '(u_id ="'.$admin_id.'")';
	...
	public function update_banq_hall_details()
    {
        ...
        $banquet_data = $this->MainModel->getData('banquet_hall_images', $wh);
		...
		if (isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])) {
            ...
            if ($this->upload->do_upload('my_file')) {
                ...
                $up = $this->MainModel->editData('banquet_hall_images', $wh, $imgarr);
            } else {
        ...
        $up = $this->MainModel->edit_data('banquet_hall', $wh, $arr);
		$data['banq_hall'] = $this->MainModel->getAllData1($tbl ='banquet_hall', $where);
  	...
  	public function update_profile()
	{
		...
		if($edit)
		{
			...
			if($get_hotel_id['user_type'] == 3){
				$uid= $this->input->post('id');           
				$profile_img= $this->MainModel->getData($tbl='register', $wh); 
				$data['admin_details'] = $this->MainModel->getData($tbl ='register', $wh);
	...
	public function getrequirement()
	{
		...
		$data["new_order_list"] = $this->FoodAdminModel->get_menu_new_order_list_pagination_food_admin($todays_date,$hotel_id,$id);
	...
	public function getEditdata_menulist()
	{
		...
		$wh1 = '(food_facility_id ="'.$data['food_facility_id'].'" AND hotel_id ="'.$data['hotel_id'].'")';
	...
	public function get_view_completed_data()
	{
		$food_order_id = $this->input->post('id');
		$wh_l = '(food_order_id = "' . $food_order_id . '")';
		$data['get_food_orders_data'] = $this->MainModel->getAllData1('food_order_details',$wh_l);
	...
	public function get_view_accepted_data()
	{
		$food_order_id = $this->input->post('id');
		$wh_l = '(food_order_id = "' . $food_order_id . '")';
		$data['get_food_orders_data'] = $this->FoodAdminModel->getAllData1('food_order_details',$wh_l);
		$this->load->view('foodadmin/ajaxview_modal_accept', $data);
	}

	// public function sendNotification() 
	// {
	//   // Load the Firebase configuration from the config file
	//   $firebase_config = $this->config->item('firebase_config');
  
	//   // Initialize Firebase with the configuration (using Firebase Admin SDK)
	//   require APPPATH . 'libraries/firebase/vendor/autoload.php';
	//   $firebase = \Firebase\Firebase::initialize($firebase_config);
	...
	{ 
		...
		$admin_id = $admin_details['hotel_id'];
		$today_date = date('Y-m-d');
		$service_request['list'] = $this->FoodAdminModel->get_service_request($u_id);
        $service_request['list2'] = $this->FoodAdminModel->get_food_service_request($admin_id,$today_date);
		$service_request['list3'] = $this->FoodAdminModel->get_accepted_service_request($admin_id);
        $service_request['list4'] = $this->FoodAdminModel->get_rejected_service_request($admin_id);
		$this->load->view('include/header');
		$this->load->view('foodadmin/viewServiceRequest',$service_request);
		$this->load->view('include/footer');
	}
    ...
    {   
        ...
        $user_data = $this->FoodAdminModel->get_user_data_by_username($user_n);
        if(!empty($user_data ))
        {
            $arr = array(
                            'hotel_id'    => $admin_id,
            ...
            $add = $this->FoodAdminModel->insert_data('service_request',$arr);
            if($add){
				$send_for = 7;
				$send_to = 2;
				...
				$add11 = $this->FoodAdminModel->insert_data('notifications',$arr);
				if($add11){
					$arr = array(
					...
					$add11 = $this->FoodAdminModel->insert_data('notifictions_department_id',$arr);
					...
					$admin_id = $admin_details['hotel_id'];
					$service_request['list'] = $this->FoodAdminModel->get_service_request($u_id);
					$this->load->view('foodadmin/ajaxviewServiceRequest',$service_request);
				}
            }
            else
            {
                $this->session->set_flashdata('msg','Something went wrong');
                redirect(base_url('request_service'));
            }
		}
		else
		{
	...
	public function handover_staff()
	{
		$admin_id = $this->session->userdata('u_id');
		$wh_admin = '(u_id ="'.$admin_id.'")';
		...
		$data["pending"] = $this->FoodAdminModel->get_staff_list_pagination($hotel_id,$todays_date);
		$data["completed"] = $this->FoodAdminModel->get_completed_staff_list_pagination($hotel_id);
		$this->load->view('include/header');
		$this->load->view('foodadmin/staff_handover',$data);
		$this->load->view('include/footer');
	}
	...
	public function getreassigndata()
	{
		...
		$data = $this->FoodAdminModel->getdatareassign($tbl ='handover_staff', $wh);
		echo json_encode($data);
	}
	...
	public function reassign_status()
	{
		...
		$get_staff_id = $this->FoodAdminModel->getData('register',$staff_assign);
		$wh1 = '(staff_id ="' . $created_staff_id . '" AND accept_date ="' . $todays_date . '" AND order_status = 1)';
		$get_food_staff_orders = $this->FoodAdminModel->getAllData1('food_orders', $wh1);
		...
		if ($get_food_staff_orders) {
			...
			$add = $this->FoodAdminModel->edit_data_assign_handover('food_orders', $arr, $wh1);
			...
			if($add){
				...
				$up = $this->FoodAdminModel->edit_data('handover_staff',$wh_hando,$arr_2);
				if($up){
					...
					$data["pending"] = $this->FoodAdminModel->get_staff_list_pagination($hotel_id,$todays_date);
					$data["completed"] = $this->FoodAdminModel->get_completed_staff_list_pagination($hotel_id);
					$this->load->view('foodadmin/ajaxstaffreassign',$data);
				}
				else{
					redirect(base_url('handover_staff'));
				}
			}
			else{
				redirect(base_url('handover_staff'));
	...
	public function get_service_request()
	{
		...
		$wh = $this->input->post('id');
		$data = $this->FoodAdminModel->get_data_service($tbl = 'service_request', $wh);
		echo json_encode($data);
	}
	...
	public function change_service_request_status()
	{
		...
		if ($request_status == 1) {
			...
			$up = $this->FoodAdminModel->edit_data('service_request', $wh, $arr);
			$up = $this->FoodAdminModel->edit_data($tbl="notifications",$where2,$arr2);
			...
			if ($up) {
				...
				$service_request['list3'] = $this->FoodAdminModel->get_accepted_service_request($admin_id);
				$service_request['list4'] = $this->FoodAdminModel->get_rejected_service_request($admin_id);
				$this->load->view('foodadmin/ajax_service_request', $service_request);
			...
			$where2 = '(ser_id ="' . $service_request_id . '")';
			$up = $this->FoodAdminModel->edit_data('service_request', $wh1, $arr1);
			$up = $this->FoodAdminModel->edit_data($tbl="notifications",$where2,$arr2);
	...
	public function reassign_staff_details()
	{
		$food_order_id=$this->input->post('food_order_id');
		$wh_admin = '(food_order_id ="'.$food_order_id.'")';
		$data['record'] = $this->FoodAdminModel->getData('food_orders',$wh_admin);

		$admin_id = $this->session->userdata('u_id');
		$wh_admin = '(u_id ="'.$admin_id.'")';
        $get_hotel_id = $this->MainModel->getData('register',$wh_admin);
      	$hotel_id = $get_hotel_id['hotel_id'];
		$wh1 = '(hotel_id ="'.$hotel_id.'" AND user_type = 8 AND is_active = 1)';
		$data['staff_record'] = $this->MainModel->getAllData($tbl ='register', $wh1);
